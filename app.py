...
def header():
 ...
 with col1:
  try:
   ...
   try:
    st.image("logo_ana.png", width=130)
   except:
...
if st.session_state.map_expand or st.session_state.map_all_expand:
 ...
 if scelta=='Dashboard':
  ...
  st.markdown('<p style="font-family:Times New Roman; color:#000000; font-weight:bold;">Clicca sulla mappa dove vuoi - lascia icona verde e compila automatico Comune/Via/Lat/Lon nella maschera</p>',unsafe_allow_html=True)
  icon_names=[]
  ...
  new_tipo=st.text_input('Nuova tipologia (memorizza)')
  ...
  c1,c2=st.columns([3,1])
  with c1: st.caption('CLICCA SULLA MAPPA - Lascia icona verde e compila automatico Via/Comune/Lat/Lon')
  with c2:
  ...
  if HAS_FOLIUM:
   ...
   for idx in range(len(st.session_state.post)):
    ...
    try:
     ...
     pass
   if st.session_state.clicked_lat is not None:
    folium.Marker([st.session_state.clicked_lat, st.session_state.clicked_lon], popup='NUOVA - Compila maschera sotto', icon=folium.Icon(color='green', icon='ok')).add_to(m)
  ...
  st.divider()
  st.markdown('##### 2 - MASCHERA VERDE ANA - INS
