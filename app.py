...
if scelta=='Dashboard':
 ...
 torna()
 st.markdown('#### MAPPA - ESPANSIONE TUTTO SCHERMO + VEDI POSTAZIONE')

 # Tipo mappa + espansione tutto schermo
 c1,c2,c3=st.columns([2,1,1])
 with c1:
  tipo_map=st.radio('Tipo mappa', ['OpenStreetMap','Google Maps','Google Earth','Waze'], horizontal=True, key='tipo_map')
 with c2:
  if st.button('⛶ ESPANDI TUTTO SCHERMO' if not st.session_state.map_expand else '⛶ RIDUCI MAPPA', use_container_width=True):
   st.session_state.map_expand=not st.session_state.map_expand
   st.rerun()
 with c3:
  if st.session_state.map_expand:
   st.success('Mappa a tutto schermo - 95% pagina')
  else:
   st.info('Mappa normale - clicca per tutto schermo')

 # Altezza mappa - tutto schermo = 900px
 map_h=900 if st.session_state.map_expand else 450
 map_w=1400 if st.session_state.map_expand else 700
 ...
 # Se hai selezionato una postazione dalla tabella, centra mappa li
 ...
 if st.session_state.sel_idx>=0 and st.session_state.sel_idx<len(st.session_state.post):
  ...
  try:
   ...
   st.info(f'📍 Stai visualizzando: {highlight_name} - {p_sel.get("Comune","")} {p_sel.get("Via","")}')
  except:
   pass

 # MAPPA PRINCIPALE
 st.markdown('##### MAPPA CLICCABILE - CLICCA PER COMPILARE MASCHERA')

 if tipo_map=='OpenStreetMap':
  if HAS_FOLIUM:
   m=folium.Map(location=[lat_c,lon_c], zoom_start=16, tiles='OpenStreetMap')
   # Marker nuova postazione con icona libreria
   ic=st.session_state.icone[sel_idx] if st.session_state.icone else {'col':'green','file':''}
   fp=ic.get('file','')
   if fp and os.path.exists(fp):
    folium.Marker([lat_c,lon_c], popup=f'Nuova - {highlight_name}', tooltip='Posizione attuale', icon=folium.CustomIcon(fp, icon_size=(32,32))).add_to(m)
   else:
    folium.Marker([lat_c,lon_c], popup=f'Nuova - {highlight_name}', tooltip='Posizione attuale', icon=folium.Icon(color=ic.get('col','green'), icon='plus')).add_to(m)
   # Tutte le postazioni
   for idx,p in enumerate(st.session_state.post):
    try:
     ...
     if idx==st.session_state.sel_idx:
      folium.Marker([la,lo], popup=f"📍 {p.get('Postazione','')} - SELEZIONATA", tooltip=f"SELEZIONATA: {p.get('Postazione','')}", icon=folium.Icon(color='red', icon='star')).add_to(m)
     else:
      folium.Marker([la,lo], popup=f"{p.get('Postazione','')}", tooltip=p.get('Postazione',''), icon=folium.Icon(color='blue')).add_to(m)
    except:
     pass
 ...
 st.divider()
 st.markdown('##### MASCHERA - SI COMPILA AUTOMATICO DAL CLICK')
 with st.form('g'):
  c1,c2=st.columns(2)
  with c1:
   m1=st.text_input('Nome postazione *', value=highlight_name if highlight_name else '', placeholder='Es: Presidio')
  ...
  with c2:
   m5=st.text_input('Latitudine *', value=str(st.session_state.lat_tmp) if st.session_state.lat_tmp!=45.8205 or st.session_state.com_tmp else '', placeholder='Click mappa')
   m6=st.text_input('Longitudine *', value=str(st.session_state.lon_tmp) if st.session_state.lon_tmp!=8.8250 or st.session_state.com_tmp else '', placeholder='Click mappa')
 ...
 st.divider()
 st.markdown('##### TABELLA CLICCABILE - CLICCA VEDI E MOSTRA NELLA MAPPA ATTUALE')
 if st.session_state.post:
  st.dataframe(pd.DataFrame(st.session_state.post), use_container_width=True)
  st.markdown('**Clicca Vedi per visualizzare automaticamente nella mappa attuale sopra**')
  for i,p in enumerate(st.session_state.post):
   c1,c2,c3,c4=st.columns([2,1,1,1])
   with c1:
    if i==st.session_state.sel_idx:
     st.markdown(f"📍 **{p.get('Postazione','')}** - {p.get('Comune','')} {p.get('Via','')} - **SELEZIONATA**")
    else:
     st.write(f"**{p.get('Postazione','')}** - {p.get('Comune','')} {p.get('Via','')}")
   with c2:
    if st.button('Vedi su mappa attuale', key=f'vedi_{i}'):
