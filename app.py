    ...
    with c1:
        try:
            ...
            try:
                ...
                st.markdown('**ANA**')
    with c2:
        st.markdown(f'<div style="background:{VERDE};padding:12px;border-radius:8px;color:white;font-weight:bold;font-size:18px;">NUCLEO PROT CIVILE ANA VARESE - Squadra Alpini Caronno - 950+ RIGHE + ICONA INTERVENTI</div>', unsafe_allow_html=True)
...
if st.session_state.page == 'entra':
    ...
    with st.sidebar:
        try:
            ...
            pass
        st.markdown('### MENU 950+ - ICONA INTERVENTI')
        try:
    ...
    if cur == 'Dashboard':
        hdr_form('Dashboard - 950+ RIGHE - TASTI RAPIDI OK - ICONA INTERVENTI')
        ...
        st.divider()
        st.markdown('### MENU RAPIDO - TASTI RAPIDI - 950+')
        r1 = st.columns(4)
        with r1[0]:
        ...
        hdr_form('DB RADIO - MASCHERA COMPLETA - 950+ RIGHE RIPRISTINATA')
        ...
        hdr_form('ALIAS RADIO - EVENTO COMBO DA FORM EVENTO OK - 950+')
        ...
        hdr_form('BROGLIACCIO - BLINDATO SU EVENTO/EMERGENZA - 950+')
        ...
        hdr_form('CHECK-IN - BLINDATO SU EVENTO/EMERGENZA - 950+ RIGHE')
        ...
        hdr_form('INTERVENTI EMERGENZA - CON ICONA DA LIBRERIA - 950+ RIGHE - NOVITA RICHIESTA')
        ...
        if st.session_state.interventi_blindato:
            with st.form('form_int'):
                ...
                with c6:
                    # --- NOVITA: ICONA DA LIBRERIA ---
                    if st.session_state.icone:
                        lista_icone = ['Nessuna'] + [i['Nome'] for i in st.session_state.icone]
                        icona_int = st.selectbox('ICONA INTERVENTO * (da Libreria Icone)', lista_icone)
                        if icona_int!= 'Nessuna':
                            ...
                            if ico_sel and ico_sel.get('FileBytes'):
                                st.image(ico_sel['FileBytes'], width=60, caption=f'Icona: {icona_int}')
                    else:
                        st.warning('Carica prima icone in Libreria Icone')
                        icona_int = st.selectbox('ICONA', ['Nessuna'])
                azione_int = st.text_area('Azione Intervento *', height=120)
                note_int = st.text_input('Note')
                if st.form_submit_button('SALVA INTERVENTO CON ICONA COLLEGATO A EMERGENZA BLINDATA', use_container_width=True, type='primary'):
                    if comune_int and via_int and azione_int:
                        iv = {'Data':str(data_int),'Ora':str(ora_int),'EmergenzaBlindata':st.session_state.interventi_emergenza_blindata,'Comune':comune_int,'Via':via_int,'Civico':civico_int,'ODV':odv_int,'Stato':stato_int,'Priorita':priorita_int,'Icona':icona_int if 'icona_int' in locals() else 'Nessuna','Azione':azione_int,'Note':note_int}
                        st.session_state.interventi.append(iv)
                        st.success(f'Intervento {stato_int} con icona {iv["Icona"]} salvato collegato a Emergenza')
                        st.balloons()
        if st.session_state.interventi:
            st.divider()
            st.markdown('### Elenco Interventi con Icona dalla Libreria')
                ...
                with c1:
                    ...
                    if icona_nome!= 'Nessuna':
                        ...
                        if ico and ico.get('FileBytes'):
                            ...
                            st.write(f'📍 {icona_nome}')
                    else:
                        st.write('📍')
                with c2:
                    st.write(f"**{interv.get('Comune','')} {interv.get('Via','')}** - {interv.get('Azione','')[:60]}")
                    st.caption(f"Stato: {interv.get('Stato','')} - Icona: {interv.get('Icona','')} - {interv.get('EmergenzaBlindata','')}")
                with c3:
                    st.write(f"{interv.get('Data','')} {interv.get('Ora','')}")
                    st.caption(f"Priorita: {interv.get('Priorita','')}")
                with c4:
            ...
            c1,c2 = st.columns(2)
            c1.download_button('Excel Interventi con Icona', to_excel(df_int), file_name='interventi_con_icona.xlsx', mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', use_container_width=True)
            pdf = to_pdf(df_int, 'Interventi con Icona')
            if pdf:
                c2.download_button('PDF Interventi con Icona', pdf, file_name='interventi_con_icona.pdf', mime='application/pdf', use_container_width=True)
        ...
        hdr_form('MAPPA AVANZATA - 950+ RIGHE - RIPRISTINATA OK - CON INTERVENTI + ICONA')
        st.info('OSM default, fullscreen + - ESC, click coord, icona marker, reverse geocoding - LAVORO MAPPA 950+')
        ...
        with st.form('form_post'):
            ...
            icona_p = st.selectbox('Icona Postazione (da Libreria)', lista_icone)
        ...
        with st.form('icone_form'):
            ...
            if st.form_submit_button('Salva Icona', type='primary', use_container_width=True):
                if nome_i and file_i:
                    ...
                    st.success(f'Icona {nome_i} caricata - ora disponibile in Interventi Emergenza e Mappa!')
                    st.balloons()
        if st.session_state.icone:
            st.divider()
            st.markdown(f'### {len(st.session_state.icone)} icone caricate - Disponibili in Interventi')
            cols = st.columns(4)
        ...
        hdr_form('BACKUP - 950+ RIGHE - EXPORT + IMPORT PER TUTTI I DATI + BACKUP DI TUTTO')
        datasets = {
        ...
        st.info(f'Totale record 950+: {totale} in {len([k for k,v in datasets.items() if v])} form')
        c1,c2,c3 = st.columns(3)
        with c1:
            if any(datasets.values()):
                st.download_button('📥 EXPORT TOTALE EXCEL - TUTTI I FORM', to_excel_multi(datasets), file_name=f'backup_TUTTO_950_{date.today()}.xlsx', mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', use_container_width=True, type='primary')
        with c2:
        ...
        st.divider()
        st.markdown('### 2. IMPORT TOTALE - RIPRISTINA TUTTI I DATI DEI FORM - 950+')
        ...
        if up_tot:
            try:
                ...
                if st.button('🔄 IMPORTA TUTTO - RIPRISTINA 950+ RIGHE', type='primary', use_container_width=True):
                    ...
                    st.success(f'Import totale 950+ completato!')
                    st.balloons()
                    st.rerun()
            except Exception as e:
        ...
        hdr_form('ESPORTA / IMPORTA PER OGNI FORM - TUTTI I DATI - 950+ RIGHE')
        for key, titolo in [('volontari','Volontari'),('radio_db','Radio'),('alias_radio','Alias Radio'),('brogliaccio','Brogliaccio'),('eventi','Eventi'),('emergenze','Emergenze'),('checkin','Check-in'),('interventi','Interventi Emergenza con Icona'),('mezzi','Mezzi'),('attrezzature','Attrezzature'),('postazioni','Postazioni Mappa'),('icone','Libreria Icone')]:
