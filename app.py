...
if not st.session_state.popup:
    hdr()
    st.markdown("<div style='background:#e8f5e9;padding:20px;border-radius:10px;border:2px solid #0e7a3d;text-align:center;'><h3 style='color:#0e7a3d;'>Buon Lavoro!</h3></div>", unsafe_allow_html=True)
...
if sc=='Dashboard':
    ...
    st.markdown("<div style='background:#e8f5e9;padding:15px;border-radius:10px;border:2px solid #0e7a3d;'><h3>Chat Volontari - Anteprima Dashboard</h3>", unsafe_allow_html=True)
    ...
    to_dash()
    st.markdown("<div style='background:#0e7a3d;padding:10px;border-radius:8px;color:white;text-align:center;'><b>VOLONTARI - CON ALETTE SOPRA</b></div>", unsafe_allow_html=True)
    if st.session_state.edit_idx >=0:
        ...
        st.markdown("</div>", unsafe_allow_html=True)
    st.markdown("<div style='background:#e8f5e9;padding:15px;border-radius:10px;border:2px solid #0e7a3d;'>", unsafe_allow_html=True)
    st.markdown("#### MASCHERA CON ALETTE SOPRA - COME IERI")
    ...
    to_dash()
    st.markdown("<div style='background:#0e7a3d;padding:10px;border-radius:8px;color:white;text-align:center;'><b>MAPPA - SALVA TUTTE LE POSIZIONI + CANCELLA + COMUNE/VIA/LAT/LON</b></div>", unsafe_allow_html=True)
    st.markdown("<div style='background:#e8f5e9;padding:15px;border-radius:10px;border:2px solid #0e7a3d;'>", unsafe_allow_html=True)
    c1,c2=st.columns(2)
    with c1:
    ...
    if HAS_MAP:
        try:
            ...
            if mp and mp.get('last_clicked'):
                ...
                st.success(f"Cliccata: Comune {st.session_state.sel_comune} Via {st.session_state.sel_via} Lat {st.session_state.sel_lat[:8]} Lon {st.session_state.sel_lon[:8]} - Riportato sotto")
        except:
            st.error("Err mappa")
    st.divider()
    st.markdown("<div style='background:#e8f5e9;padding:15px;border-radius:10px;border:2px solid #0e7a3d;'><h3>Maschera sotto - Salva tutte le posizioni + tasto cancella</h3>", unsafe_allow_html=True)
    st.markdown("#### Cliccato riporta comune, via, lat, long in maschera")
        ...
        with st.form("mapa",clear_on_submit=True):
            ...
            if bm:
                if m_nome and m_com:
                    ...
                    save_json(FP,st.session_state.post)
                    st.success(f"OK {m_nome} salvata - Puoi salvarne altre")
                    st.balloons()
                    st.rerun()
    st.markdown("</div>", unsafe_allow_html=True)
    st.divider()
    ...
    if st.session_state.post:
        st.write(f"Totale: {len(st.session_state.post)} posizioni - Salva tutte")
        ...
        st.info("Nessuna postazione - Clicca mappa sopra")
    st.markdown("</div>", unsafe_allow_html=True)
    ...
    to_dash()
    st.markdown("<div style='background:#0e7a3d;padding:10px;border-radius:8px;color:white;text-align:center;'><b>CHECK IN - VERDE ANA</b></div>", unsafe_allow_html=True)
    st.markdown("<div style='background:#e8f5e9;padding:15px;border-radius:10px;border:2px solid #0e7a3d;'>", unsafe_allow_html=True)
    vlist=[d.get('Nome','') for d in st.session_state.dati]
    if vlist:
    ...
    to_dash()
    st.markdown("<div style='background:#0e7a3d;padding:10px;border-radius:8px;color:white;text-align:center;'><b>DB RADIO - VERDE ANA</b></div>", unsafe_allow_html=True)
    st.markdown("<div style='background:#e8f5e9;padding:15px;border-radius:10px;border:2px solid #0e7a3d;'>", unsafe_allow_html=True)
    ...
    to_dash()
    st.markdown("<div style='background:#0e7a3d;padding:10px;border-radius:8px;color:white;text-align:center;'><b>CONSEGNA RADIO - VERDE ANA</b></div>", unsafe_allow_html=True)
    st.markdown("<div style='background:#e8f5e9;padding:15px;border-radius:10px;border:2px solid #0e7a3d;'>", unsafe_allow_html=True)
    vlist=[d.get('Nome','') for d in st.session_state.dati]
    ...
    to_dash()
    st.markdown("<div style='background:#0e7a3d;padding:10px;border-radius:8px;color:white;text-align:center;'><b>CHAT VOLONTARI - VERDE ANA</b></div>", unsafe_allow_html=True)
    st.markdown("<div style='background:#e8f5e9;padding:15px;border-radius:10px;border:2px solid #0e7a3d;'>", unsafe_allow_html=True)
    c1,c2=st.columns([3,1])
    with c1: st.write(f"Utente: {st.session_state.chat_user}")
    with c2:
    ...
    if st.session_state.chat:
        for msg in st.session_state.chat[-20:]:
            ...
            text=msg.get('Text','')
            time=msg.get('Time','')
            if user==st.session_state.chat_user:
                st.markdown(f"<div style='background:#d9fdd3;padding:8px;border-radius:8px;margin:5px;text-align:right;'><b>{user}</b> {time}<br>{text}</div>", unsafe_allow_html=True)
            else:
                st.markdown(f"<div style='background:white;padding:8px;border-radius:8px;margin:5px;border:1px solid #0e7a3d;'><b>{user}</b> {time}<br>{text}</div>", unsafe_allow_html=True)
    else:
        st.info("Nessun messaggio")
    st.markdown("</div>", unsafe_allow_html=True)
    st.markdown("<div style='background:#e8f5e9;padding:15px;border-radius:10px;border:2px solid #0e7a3d;'>", unsafe_allow_html=True)
    with st.form("chat_form",clear_on_submit=True):
        chat_text=st.text_input("Scrivi messaggio")
        chat_send=st.form_submit_button("INVIA IN CHAT")
        if chat_send:
            if chat_text:
    ...
    to_dash()
    st.markdown("<div style='background:#0e7a3d;padding:10px;border-radius:8px;color:white;text-align:center;'><b>BACKUP - VERDE ANA</b></div>", unsafe_allow_html=True)
    st.markdown("<div style='background:#e8f5e9;padding:15px;border-radius:10px;border:2px solid #0e7a3d;'>", unsafe_allow_html=True)
    ...
    to_dash()
    st.markdown("<div style='background:#0e7a3d;padding:10px;border-radius:8px;color:white;text-align:center;'><b>TESSERINO - VERDE ANA</b></div>", unsafe_allow_html=True)
    st.markdown("<div style='background:#e8f5e9;padding:15px;border-radius:10px;border:2px solid #0e7a3d;'>", unsafe_allow_html=True)
    if st.session_state.dati:
        df=pd.DataFrame(st.session_state.dati)
