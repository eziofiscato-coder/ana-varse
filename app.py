...
def make_pdf_simple(title, df):
 # FALLBACK PDF SENZA REPORTLAB - funziona sempre
 try:
  lines=[]
  lines.append(title)
  lines.append(f"Data: {datetime.now().strftime('%d/%m/%Y %H:%M')} - ANA Varese Protezione Civile")
  lines.append("")
...
def make_pdf(title, df):
 # Prova reportlab, se manca usa fallback
 try:
  ...
  # reportlab non installato -> usa fallback
  return make_pdf_simple(title, df)
 except Exception as e:
  # Altro errore -> prova fallback
  try:
   return make_pdf_simple(title, df)
  except:
...
if st.session_state.exp1 or st.session_state.exp2 or st.session_state.exp3:
 ...
 if scelta=='Dashboard':
   ...
   with c2:
    if st.button('CREA PDF VOLONTARI',key='pdf_vol_btn'):
     ...
     if pdf:
      ...
      st.success('PDF creato correttamente! (fallback se reportlab manca)')
  ...
  with tab_exp:
   st.markdown('##### EXPORT EXCEL PER SINGOLI FORM - TUTTI')
   c1,c2,c3=st.columns(3)
   with c1:
