...
def reverse_geocode(lat, lon):
    try:
        ...
        if r.status_code==200:
            ...
            return via, comune, data.get("display_name","")
    except:
        pass
    return "", "", ""
...
for k,v in [("dati",[]),("postazioni",[]),("icone_lib",[]),("menu_scelta","Dashboard"),("map_lat",45.8205),("map_lon",8.8250),("map_via",""),("map_comune","Varese"),("map_tipo","StreetMap"),("map_logo","Default"),("post_sel",None),("authenticated",False)]:
...
if scelta=="Dashboard":
    st.markdown("## 🏠 DASHBOARD - MENU COMPLETO TUTTI I FORM")
    c1,c2,c3=st.columns(3)
    with c1:
        if st.button("👥 VOLONTARI\n6 Sottomaschere",key="dash_vol",use_container_width=True):
            st.session_state.menu_scelta="Volontari"
            st.rerun()
    with c2:
        if st.button("📍 MAPPA POSTAZIONI\nAnteprima tutte + Loghi 10x10",key="dash_mappa",use_container_width=True):
    ...
    torna_dashboard()
    st.markdown("## 📍 MAPPA POSTAZIONI - ANTEPRIMA TUTTE + VIA CON LAT/LON + LOGHI 10x10")
    st.caption("Come richiesto: Anteprima tutte le postazioni salvate | Maschera campo Via con via selezionata come lat/lon | Loghi 10x10 puntatori piccolissimi")

    st.markdown("### 🎨 Libreria loghi - Loghi 10x10 come puntatori")
        ...
        for i, ic in enumerate(st.session_state.icone_lib):
            with cols[i%6]:
                ...
                if ic.get("b64"):
                    ...
                    st.write(f"**{ic['nome']}**")
                    st.caption("10x10 su mappa")
                    if st.button(f"👁️ VEDI",key=f"vedi_lib_{i}_{ic['nome']}",use_container_width=True):
    ...
    with c1:
        st.markdown("### 🗺️ MAPPA PER SELEZIONARE - Clicca e inserisce Via + Lat/Lon in maschera")
        try:
            import folium
            ...
            for idx_p, p in enumerate(st.session_state.postazioni):
                try:
                    ...
                    if b64:
                        ...
                        if tmp_path:
                            icon=folium.CustomIcon(tmp_path, icon_size=(10,10))
            ...
            if map_data and map_data.get("last_clicked"):
                ...
                via_auto, comune_auto, display=reverse_geocode(lat_c, lon_c)
                if via_auto:
                    st.session_state.map_via=f"{via_auto} - {lat_c:.6f}, {lon_c:.6f}"
                else:
                    st.session_state.map_via=f"{lat_c:.6f}, {lon_c:.6f}"
                if comune_auto:
                    st.session_state.map_comune=comune_auto
                st.toast(f"Via: {via_auto} - Lat/Lon: {lat_c:.6f},{lon_c:.6f}")
                st.rerun()
        except Exception as e:
            st.error(f"Errore: {e}")

        st.markdown("### 🔍 ANTEPRIMA - VEDI TUTTE LE POSTAZIONI SALVATE")
        st.markdown("#### Scegli mappa per vedere TUTTE le postazioni salvate:")
        ...
        try:
            ...
            elif st.session_state.postazioni:
                lat_center=float(st.session_state.postazioni[-1].get('Latitudine',lat_center))
                lon_center=float(st.session_state.postazioni[-1].get('Longitudine',lon_center))

            if tipo_mappa=="OpenTopoMap (OTM)":
                m2=folium.Map(location=[lat_center, lon_center], zoom_start=14, tiles="OpenTopoMap")
            elif tipo_mappa=="Google Maps Stradale":
                m2=folium.Map(location=[lat_center, lon_center], zoom_start=14, tiles="https://mt1.google.com/vt/lyrs=m&x={x}&y={y}&z={z}", attr="Google")
            elif tipo_mappa=="Google Satellite":
                m2=folium.Map(location=[lat_center, lon_center], zoom_start=14, tiles="https://mt1.google.com/vt/lyrs=s&x={x}&y={y}&z={z}", attr="Google")
            elif tipo_mappa=="Waze Style":
                m2=folium.Map(location=[lat_center, lon_center], zoom_start=14, tiles="CartoDB positron")
            else:
                m2=folium.Map(location=[lat_center, lon_center], zoom_start=14)
            ...
            st.success(f"ANTEPRIMA: {tipo_mappa} - VEDI TUTTE {len(st.session_state.postazioni)} postazioni salvate - Loghi 10x10 puntatori - Visualizzata NEL FORM")
        except Exception as e:
            st.error(f"Errore anteprima: {e}")

    with c2:
        st.markdown("### 📋 Maschera - Via + Lat/Lon + Campo Selezionata")
        st.info(f"Lat {st.session_state.map_lat:.6f} Lon {st.session_state.map_lon:.6f}")
        if st.session_state.map_via:
            st.success(f"Via selezionata con Lat/Lon: {st.session_state.map_via}")
        if st.session_state.post_sel:
            p_sel=st.session_state.post_sel
            st.success(f"Selezionata: {p_sel.get('Postazione','')}\nVia: {p_sel.get('Via','')}\nComune: {p_sel.get('Comune','')}")

        st.write(f"Logo: {st.session_state.map_logo} - 10x10 su mappa")
        ...
        sel_logo=st.selectbox("Scegli logo 10x10", opzioni_logo, index=idx_default, key="sel_logo_mask")
        ...
        with st.form("form_post_finale"):
            ...
            nome_post=st.text_input("Nome Postazione * - Selezionata",value=val_nome,key="inp_nome")
            ...
            comune_post=st.selectbox("Comune * - Selezionata",COMUNI,index=idx_comune,key="inp_comune")

            val_via=st.session_state.map_via
            if st.session_state.post_sel:
                val_via=st.session_state.post_sel.get('Via','')
            via_post=st.text_input("Via * - Via selezionata come Lat/Lon + Selezionata",value=val_via,key="inp_via",help="Quando clicchi su mappa inserisce Via + Latitudine Longitudine come richiesto")
            ...
            salva_post=st.form_submit_button("📍 SALVA CON VIA LAT/LON E LOGO 10x10",use_container_width=True,type="primary")
            if salva_post:
                if nome_post and lat_post and lon_post:
                    ...
                    st.success(f"Salvata {nome_post} - {via_post} - Logo 10x10 - Visibile in anteprima tutte!")
                    st.rerun()

    st.divider()
    st.markdown("### 📋 TABELLA POSTAZIONI - Loghi 10x10")
    if st.session_state.postazioni:
        ...
        for idx, p in enumerate(st.session_state.postazioni):
            ...
            with c_img:
                ...
                b64=trova_b64_logo(logo_nome, st.session_state.icone_lib)
                if b64:
                    st.image(f"data:image/png;base64,{b64}",width=10)
                else:
                    st.write(logo_nome)
            with c1:
                st.write(f"**{p.get('Postazione','')}**")
                st.caption(f"Logo 10x10: {logo_nome}")
            with c2:
                st.write(f"{p.get('Via','')}")
                st.caption(f"{p.get('Comune','')}")
            with c3:
