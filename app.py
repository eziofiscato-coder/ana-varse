...
.stForm label{color:#000!important;font-weight:bold!important;font-family:Times New Roman!important;font-size:16px!important;}
.stTextInput input{color:#000!important;font-weight:bold!important;font-family:Times New Roman!important;}
.stSelectbox div{color:#000!important;font-weight:bold!important;}
...
if not st.session_state.odv:
 st.session_state.odv=['ANA Varese','Protezione Civile','Croce Rossa','Alpini','AIB','Altro']
...
with st.sidebar:
 st.markdown('<b style="font-family:Times New Roman;">MENU</b>', unsafe_allow_html=True)
...
if scelta=='Dashboard':
 ...
 with c1:
  st.info('Tutti i form disponibili - clicca per entrare')
 with c2:
  if st.button('LOGOUT',use_container_width=True):
   st.session_state.auth=False
   st.rerun()
 ...
 st.write('- Volontari: inserimento anagrafica')
 st.write('- Mappa: inserimento postazioni con Via/Comune/Lat/Lon automatico + ODV operante + icona')
 st.write('- Libreria Icone: gestione icone')
 st.write('- Backup: scarica Excel')
 ...
 with st.form('vol'):
  ...
  if st.form_submit_button('SALVA VOLONTARIO'):
   if a1 and a2 and a4:
    nc=a1+' '+a2
    nuovo={'Nome':nc,'Cellulare':a4,'Ruolo':a3}
 ...
 if st.session_state.dati:
  ...
  st.download_button('SCARICA EXCEL VOLONTARI',out.getvalue(),file_name='volontari.xlsx')

elif scelta=='Mappa':
 torna()
 st.markdown('<h3 style="font-family:Times New Roman;color:#000;font-weight:bold;">MAPPA - DECIDI TU LA POSTAZIONE</h3>', unsafe_allow_html=True)
 st.caption('Clicca sulla mappa dove vuoi - lascia icona verde e compila automatico Via/Comune/Lat/Lon nella maschera')

 icon_names=[]
 for it in st.session_state.icone:
  icon_names.append(it.get('nome','')+' ('+it.get('col','')+')')
 sel_idx=0
 if icon_names:
 ...
 new_t=st.text_input('Nuova tipologia (memorizza)')
 if st.button('AGGIUNGI TIPOLOGIA'):
  if new_t and new_t not in st.session_state.tip:
   st.session_state.tip.append(new_t)
   save(FT,st.session_state.tip)
   st.success('Memorizzata '+new_t)
   st.rerun()

 st.divider()
 st.markdown('##### 1 - MAPPA - CLICCA DOVE VUOI - LASCIA ICONA VERDE')
 ...
 if st.session_state.sel>=0:
  try:
   ...
   zm=19
   st.success('INGRANDITA: '+p.get('Postazione',''))
  except:
   pass
 if HAS:
  ...
  if st.session_state.clat is not None:
   folium.Marker([st.session_state.clat,st.session_state.clon],popup='NUOVA POSIZIONE',icon=folium.Icon(color='green')).add_to(m)
 ...
 st.divider()
 st.markdown('##### 2 - MASCHERA VERDE ANA - VIA/COMUNE/LAT/LON AUTOMATICI + ODV')
 st.success('Clicca mappa sopra - i campi sotto si riempiono da soli! NERO GRASSETTO')
 vol_nomi=[]
 ...
 with st.form('g'):
  c1,c2=st.columns(2)
  with c1:
   m1=st.text_input('Nome postazione *',placeholder='Decidi tu nome')
  ...
  with c2:
   ...
   m8_new=st.text_input('Nuovo responsabile')
  ...
  if st.form_submit_button('SALVA POSTAZIONE CON ICONA'):
   if m1:
    ...
    if m8_new and m8_new not in vol_nomi:
     st.session_state.dati.append({'Nome':m8_new})
     save(FD,st.session_state.dati)
    ...
    nuovo={'Postazione':m1,'Comune':m2,'Via':m3,'Lat':m5,'Lon':m6,'Tipo':tf,'Resp':rf,'ODV':odv_f,'Col':ic.get('col','blue')}
 ...
 if st.session_state.post:
  ...
  if st.session_state.prev>=0:
   try:
    ...
    st.markdown('#### ANTEPRIMA MAPPA - SENZA VELO ROSSO')
    st.success('Anteprima: '+p_prev.get('Postazione','')+' - '+p_prev.get('Comune',''))
  ...
  for i,p in enumerate(st.session_state.post):
   ...
   with c1:
    st.write(p.get('Postazione','')+' - '+p.get('Comune','')+' - '+p.get('ODV',''))
   with c2:
 ...
 torna()
 st.markdown('#### BACKUP')
 st.write('Scarica tutti i dati in Excel')
 ...
 if 'bk' in st.session_state:
  st.download_button('SCARICA BACKUP EXCEL',st.session_state['bk'],file_name='BACKUP_ANA_VARESE.xlsx',use_container_width=True)
