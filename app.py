...
if 'expand_main' not in st.session_state: st.session_state.expand_main=False
if 'expand_all' not in st.session_state: st.session_state.expand_all=False
if 'expand_prev' not in st.session_state: st.session_state.expand_prev=False
...
if st.session_state.expand_main or st.session_state.expand_all or st.session_state.expand_prev:
 ...
 if st.button('TORNA AL FORM',use_container_width=True,type='primary'):
  st.session_state.expand_main=False
  st.session_state.expand_all=False
  st.session_state.expand_prev=False
  st.rerun()

 if st.session_state.expand_main:
 ...
 if st.session_state.expand_prev:
  st.markdown('## ANTEPRIMA POSTAZIONE - SCHERMO INTERO')
  if st.session_state.prev>=0:
   try:
    p=st.session_state.post[st.session_state.prev]
    st.success('Postazione: '+p.get('Postazione','')+' - Comune: '+p.get('Comune','')+' - ODV: '+p.get('ODV',''))
    la=float(p.get('Lat','0'))
    lo=float(p.get('Lon','0'))
    if HAS:
     m3=folium.Map(location=[la,lo],zoom_start=19,tiles='OpenStreetMap')
     popup=p.get('Postazione','')+'<br>ODV: '+p.get('ODV','')+'<br>'+p.get('Comune','')
 ...
 if scelta=='Dashboard':
  ...
  torna()
  st.markdown('<h3>MAPPA - QUALSIASI COMUNE</h3>', unsafe_allow_html=True)
  ...
  with c2:
   if st.button('ESPANDI SCHERMO INTERO',use_container_width=True,key='e1'):
    st.session_state.expand_main=True
    st.rerun()
  ...
  with st.form('g'):
   ...
   with c2:
    ...
    m9_new=st.text_input('Nuova ODV')
  ...
  if st.session_state.post:
   ...
   if st.session_state.prev>=0:
    try:
     ...
     st.markdown('#### ANTEPRIMA - ODV OPERANTE')
     st.success('Postazione: '+p_prev.get('Postazione',''))
     st.info('Comune: '+p_prev.get('Comune','')+' - Via: '+p_prev.get('Via','')+' - ODV: '+p_prev.get('ODV',''))

     c1,c2,c3=st.columns([1,1,1])
     with c1:
      if st.button('ESPANDI ANTEPRIMA SCHERMO INTERO',use_container_width=True,key='exp_prev'):
       st.session_state.expand_prev=True
       st.rerun()
     with c2:
      if st.button('TORNA INDIETRO SENZA VISUALIZZARE',use_container_width=True,key='back_no_view'):
       st.session_state.prev=-1
       st.session_state.sel=-1
       st.rerun()
     with c3:
