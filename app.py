...
if scelta=="Dashboard":
    ...
    with st.form("form_post"):
        ...
        if st.form_submit_button("Aggiungi alla Mappa"):
            if nome and lat and lon:
                ...
                save_json(FILE_POST,st.session_state.postazioni); st.success("Aggiunta!"); st.rerun()
    ...
    with tab2:
        with st.container(border=True):
            ...
            with c2:
                ...
                if len(vie_cont)>10: st.success(f"{len(vie_cont)-1} vie trovate!")
    ...
    with st.form("form_radio"):
        ...
        if st.form_submit_button("Salva Radio"):
            if rid and mod:
                ...
                save_json(FILE_RADIO,st.session_state.radio_db); st.success("Aggiunta!"); st.rerun()
    ...
    with st.form("form_dist"):
        ...
        if st.form_submit_button("Assegna Radio"):
            if rid and ass!="--" and posto:
                ...
                save_json(FILE_DIST,st.session_state.dist_radio); st.success("Assegnata!"); st.rerun()
    ...
    with c2:
        ...
        if len(vie_ev)>10: st.success(f"{len(vie_ev)-1} vie - TUTTE!")
        ...
        if via_ev=="-- Seleziona Via --":
            via_man_ev=st.text_input("Via manuale",key="via_man_ev"); via_f_ev=via_man_ev if via_man_ev else via_ev
    ...
    with st.form("form_checkin"):
        ...
        if st.form_submit_button("Registra Check-in",type="primary"):
            if vol_check and evento_check:
                ...
                save_json(FILE_CHECK,st.session_state.checkin_lista); st.success("Check-in!"); st.rerun()
    ...
    torna("top_tab")
    st.markdown("### 📋 TABELLA COMPLETA INTERVENTI EMERGENZA - TUTTI I CAMPI")
    if not st.session_state.emergenze_lista:
        ...
        c1,c2,c3=st.columns(3)
        with c1: filtro_comune=st.selectbox("Filtra Comune",["Tutti"]+sorted(df["Comune"].unique().tolist()))
        with c2: filtro_tipo=st.selectbox("Filtra Tipo",["Tutti"]+sorted(df["Tipo"].unique().tolist()))
        with c3: ricerca=st.text_input("🔍 Cerca")
        df_f=df.copy()
        if filtro_comune!="Tutti": df_f=df_f[df_f["Comune"]==filtro_comune]
        if filtro_tipo!="Tutti": df_f=df_f[df_f["Tipo"]==filtro_tipo]
        if ricerca: df_f=df_f[df_f.apply(lambda row: ricerca.lower() in str(row["Descrizione"]).lower() or ricerca.lower() in str(row["Via"]).lower(), axis=1)]
        st.markdown(f"**Totale: {len(df)} | Filtrati: {len(df_f)}**")
        st.dataframe(df_f,use_container_width=True,hide_index=True)
        out=BytesIO(); df_f.to_excel(out,index=False,engine="openpyxl")
