...
def mostra_backup():
    ...
    with tab1:
        ...
        with col1:
            chk_vol = st.checkbox(f"👤 Volontari ({len(st.session_state.dati)})", value=True, key="chk_vol_gen")
            chk_post = st.checkbox(f"📍 Postazioni ({len(st.session_state.postazioni)})", value=True, key="chk_post_gen")
            chk_emer = st.checkbox(f"🚨 Emergenze ({len(st.session_state.emergenze_lista)})", value=True, key="chk_emer_gen")
            chk_radio = st.checkbox(f"📻 DB Radio ({len(st.session_state.radio_db)})", value=True, key="chk_radio_gen")
        with col2:
            chk_dist = st.checkbox(f"📡 Distribuzione Radio ({len(st.session_state.dist_radio)})", value=True, key="chk_dist_gen")
            chk_eventi = st.checkbox(f"📅 Eventi ({len(st.session_state.eventi_lista)})", value=True, key="chk_eventi_gen")
            chk_check = st.checkbox(f"✅ Check-in ({len(st.session_state.checkin_lista)})", value=True, key="chk_check_gen")
            chk_nomi = st.checkbox(f"📝 Mem Nomi ({len(st.session_state.mem_nomi)})", value=True, key="chk_nomi_gen")
        c1, c2, c3 = st.columns(3)
        with c1:
        ...
        with c3:
            ...
            st.metric("Selezionati", f"{selected_count}/8")
        st.markdown('</div>', unsafe_allow_html=True)
        ...
        if "backup_bytes_sel" in st.session_state:
            fname=f"backup_{st.session_state.backup_count_sel}form_{date.today()}.xlsx"
            st.download_button(f"📥 Scarica Excel {st.session_state.backup_count_sel} Form",st.session_state.backup_bytes_sel,file_name=fname,mime=MIME_SHORT,use_container_width=True,key="dl_excel_sel")
        ...
        if "backup_pdf_sel" in st.session_state:
            st.download_button(f"📥 Scarica PDF {st.session_state.backup_pdf_count} Form", st.session_state.backup_pdf_sel, file_name=f"backup_{st.session_state.backup_pdf_count}form_{date.today()}.pdf", mime="application/pdf", use_container_width=True, key="dl_pdf_sel")
    ...
    with tab3:
        ...
        st.info("Usa le caselle nel primo tab per scegliere i form del PDF!")
