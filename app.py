...
if scelta=="Dashboard":
    ...
    st.divider()
    st.markdown("### Elenco - Clicca per 6 sottomaschere")
    ...
    torna_dashboard()
    st.markdown("## 📍 MAPPA POSTAZIONI - STEP 2+3")
    st.info("STEP 2: Loghi scelti creano tabella con immagine | STEP 3: Clicca logo salvato -> Google/OSM/Waze")
    ...
    if st.session_state.icone_lib:
        st.markdown("#### 📚 Libreria condivisa - Seleziona logo")
        cols=st.columns(4)
        for i, ic in enumerate(st.session_state.icone_lib):
            with cols[i%4]:
                ...
                if ic.get("b64"):
                    ...
                    st.write(f"{ic['nome']}")
                    if st.button(f"👁️ Seleziona",key=f"view_post_{i}",use_container_width=True):
    ...
    with c1:
        st.markdown("### 🗺️ Mappa + Google/OSM/Waze con logo scelto")
        ...
        try:
            ...
            if map_data and map_data.get("last_clicked"):
                st.session_state.map_lat=map_data["last_clicked"]["lat"]; st.session_state.map_lon=map_data["last_clicked"]["lng"]; st.rerun()
        except:
            st.map(pd.DataFrame([{"lat":lat_focus,"lon":lon_focus}]),zoom=12)
        # STEP 3: BOTTONI GOOGLE / OSM / WAZE CON LOGO SCELTO
        st.markdown("#### 🌍 Apri su Google/OSM/Waze con logo scelto")
        if st.session_state.postazione_selezionata_per_mappa:
            p_sel=st.session_state.postazione_selezionata_per_mappa
            lat_sel=p_sel.get('Latitudine'); lon_sel=p_sel.get('Longitudine'); logo_sel=p_sel.get('Icona','📍')
            st.success(f"Logo scelto: {logo_sel} - {p_sel.get('Postazione','')} - {lat_sel},{lon_sel}")
            ...
            c_osm,c_gm,c_waze=st.columns(3)
            with c_osm: st.link_button("OSM", f"https://www.openstreetmap.org/?mlat={lat}&mlon={lon}#map=17/{lat}/{lon}", use_container_width=True)
    ...
    st.divider()
    st.markdown("### 📋 STEP 2: Tabella postazioni - Loghi come IMMAGINE - Clicca per Google/OSM/Waze")
        ...
        for idx, p in enumerate(st.session_state.postazioni):
            ...
            with c_img:
                ...
                if b64:
                    ...
                    if st.button(f"👁️",key=f"view_img_{idx}"):
                        st.session_state.postazione_selezionata_per_mappa=p; st.session_state.map_lat=float(p.get('Latitudine',45.8205)); st.session_state.map_lon=float(p.get('Longitudine',8.8250)); st.session_state.map_logo_selezionato=logo_nome; st.rerun()
                else:
            ...
            with c3:
                if st.button(f"📍 Vedi su Mappa Google/OSM/Waze",key=f"use_em_{idx}",use_container_width=True):
        ...
        for i, ic in enumerate(st.session_state.icone_lib):
            with cols[i%4]:
                ...
                if st.button(f"Seleziona",key=f"view_em_{i}",use_container_width=True):
                    st.session_state.em_logo_selezionato=ic['nome']; st.rerun()
                st.markdown('</div>',unsafe_allow_html=True)
