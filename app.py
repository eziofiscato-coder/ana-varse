...
if not st.session_state.tip:
 st.session_state.tip=[
  'Presidio','Blocco','Ritrovo',
  'Parcheggio','Sanitario','Altro'
 ]
 save(FT,st.session_state.tip)
if not st.session_state.odv:
 st.session_state.odv=[
  'ANA Varese','Prot Civile',
  'Croce Rossa','Alpini','Altro'
 ]
...
def header():
 ...
 with c2:
  st.markdown(
   ...
   "border:2px solid #0e7a3d;text-align:center;'>"
   "<b style='color:#000;font-family:Times New Roman;"
   "font-size:28px;'>VOLONTARIATO<br>Sezione di Varese</b></div>",
   unsafe_allow_html=True
  )
...
if st.session_state.exp1 or st.session_state.exp2 or st.session_state.exp3:
 ...
 if st.button('TORNA AL FORM',use_container_width=True):
 ...
 with st.sidebar:
  ...
  opts=[
   ...
   'Libreria Icone','Emergenza',
   'Check In','DB Radio','Consegna Radio','Backup'
  ]
 ...
 if scelta=='Dashboard':
  ...
  sel_idx=0
  if icon_names:
   sel_str=st.selectbox('Icona PNG',icon_names,index=0)
  ...
  with c2:
   if st.button('ESPANDI',use_container_width=True,key='e1'):
  ...
  with c2:
   st.write(f"Comune {st.session_state.com}")
  with c3:
  ...
  with st.form('g'):
   ...
   if st.form_submit_button('SALVA CON ICONA PNG'):
    if m1:
     ic=st.session_state.icone[sel_idx]
     nuovo={
      'Postazione':m1,
      'Comune':m2,
      'Via':m3,
  ...
  c1,c2=st.columns([3,1])
  with c2:
   if st.button('ESPANDI',use_container_width=True,key='e2'):
  ...
  st.divider()
  st.markdown('##### 4 - ELENCO CON ICONA PNG')
  ...
  if st.session_state.post:
   ...
   if st.session_state.prev>=0:
    try:
     p=st.session_state.post[st.session_state.prev]
     # FIX RIGA 508 - RIGHE CORTE
     nome=p.get('Postazione','')
     icona=p.get('Icona','')
     com=p.get('Comune','')
     via=p.get('Via','')
     odv=p.get('ODV','')
     st.success(f"Postazione: {nome}")
     st.write(f"Icona: {icona}")
     st.info(f"Comune: {com} - Via: {via} - ODV: {odv}")
