...
if st.session_state.page == 'entra':
    ...
    with st.sidebar:
        try:
            ...
            pass
        st.markdown('### MENU')
        # FIX TASTI DASHBOARD - indice dinamico
        try:
    ...
    if m == 'Dashboard':
        hdr_form('Dashboard - TASTI RAPIDI FIXATI OK')
        st.info('Clicca e apre il form assegnato - FIXATO')
        ...
        # MASCHERA COME IERI - 5 SOTTOMASCHERE
        hdr_form('VOLONTARI - MASCHERA COME IERI')
        t1,t2,t3,t4,t5 = st.tabs(['1.Anagrafica','2.Contatti','3.Ruolo','4.Foto','5.Elenco'])
        with t1:
            st.markdown('### Maschera Anagrafica - come ieri')
        ...
        with t2:
            st.markdown('### Maschera Contatti - come ieri')
        ...
        with t3:
            st.markdown('### Maschera Ruolo - come ieri')
            with st.form('vol_ruolo'):
                ...
                if btn3:
                    ...
                    st.success('Ruolo salvato - vai a Foto')
        with t4:
            st.markdown('### Maschera Foto - come ieri')
            ...
            with st.form('vol_foto'):
                ...
                if btn4:
                    if not st.session_state.vol_form_data.get('Nome'):
                        ...
                        st.success('Volontario salvato! Vai in Elenco')
                        st.balloons()
                        st.rerun()
        with t5:
            st.markdown('### Elenco Volontari - Tabella con volontari salvati')
            if st.session_state.volontari:
                ...
                st.success(f"Trovati {len(lista)} volontari salvati")
                ...
                st.warning('Nessun volontario - compila le 4 sottomaschere')
        ...
        hdr_form('BROGLIACCIO - MITTENTE E DESTINATARIO DA ALIAS RADIO')
        # NUOVA LOGICA - COMBO AGANCIATA AD ALIAS RADIO CAMPO NOME ALIAS
        if not st.session_state.alias_radio:
            st.warning('Crea prima Alias Radio con campo NOME ALIAS - poi appariranno qui in Mittente e Destinatario')
        st.info('Mittente e Destinatario sono combo da Alias Radio > NOME ALIAS')
        with st.form('brog_form'):
            # COMBO AGANCIATA AD FORM ALIAS CAMPO NOME ALIAS
            if st.session_state.alias_radio:
            ...
            btn_b = st.form_submit_button('Salva Brogliaccio', type='primary', use_container_width=True)
            if btn_b:
        ...
        if st.session_state.brogliaccio:
            ...
            st.dataframe(pd.DataFrame(st.session_state.brogliaccio), use_container_width=True, hide_index=True)
            st.download_button('Excel Brogliaccio', to_excel(pd.DataFrame(st.session_state.brogliaccio)), file_name='brogliaccio.xlsx', mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', use_container_width=True)
        ...
        hdr_form('CHECK-IN - MASCHERA FIXATA')
        ...
        hdr_form('ESPORTA RAPIDO')
        for key, titolo in [('volontari','Volontari'),('radio_db','Radio'),('alias_radio','Alias Radio'),('brogliaccio','Brogliaccio'),('eventi','Eventi'),('emergenze','Emergenze'),('checkin','Check-in'),('postazioni','Postazioni')]:
