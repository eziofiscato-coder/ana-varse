...
if scelta=="Dashboard":
    ...
    with c_logout1:
        st.success(f"Benvenuto {st.session_state.username} | Ruolo: {st.session_state.ruolo} | Locale e Cloud OK")
    with c_logout2:
    ...
    with mr1:
        if st.button("Volontari",key="rapido_vol",use_container_width=True):
            ...
            st.rerun()
    with mr2:
        if st.button("Mappa",key="rapido_mappa",use_container_width=True):
