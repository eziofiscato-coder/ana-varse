...
def header():
 # INTESTAZIONE STACCATA + LOGO ANA GRANDE
...
if not st.session_state.auth:
 header()
 # MASCHERA LOGIN STACCATA DALLA INTESTAZIONE
...
if st.session_state.map_expand or st.session_state.map_all_expand:
   ...
   for idx in range(len(st.session_state.post)):
    ...
    try:
     ...
     if idx==st.session_state.sel_idx:
      ...
      folium.Circle([la,lo],radius=80,color='red',fill=True,fill_opacity=0.2).add_to(m)
     else:
 ...
 if scelta=='Dashboard':
  ...
  if HAS_FOLIUM:
   ...
   for idx in range(len(st.session_state.post)):
    ...
    try:
     ...
     if idx==st.session_state.sel_idx:
      folium.Marker([la,lo],popup=p.get('Postazione',''),icon=folium.Icon(color='red',icon='star')).add_to(m)
      folium.Circle([la,lo],radius=100,color='red',fill=True,fill_opacity=0.3).add_to(m)
     else:
      folium.Marker([la,lo],popup=p.get('Postazione',''),icon=folium.Icon(color='blue')).add_to(m)
    except:
     pass
  ...
  st.divider()
  st.markdown('##### 2 - MASCHERA VERDE ANA - VIA COMUNE LAT LON + ODV')
  ...
  st.divider()
  st.markdown('##### 4 - ELENCO POSTAZIONI - CLICCA PER VEDERE ANTEPRIMA INGRANDITA SULLA MAPPA')
  if st.session_state.post:
   st.dataframe(pd.DataFrame(st.session_state.post),use_container_width=True)
   # ANTEPRIMA MAPPA QUANDO CLICCHI SU POSTAZIONE
   if st.session_state.preview_idx>=0 and st.session_state.preview_idx<len(st.session_state.post):
    ...
    st.markdown('#### ANTEPRIMA MAPPA - POSTAZIONE SELEZIONATA CON INGRANDIMENTO')
    ...
    try:
     ...
     if HAS_FOLIUM:
      ...
      folium.Circle([la_prev,lo_prev],radius=80,color='red',fill=True,fill_opacity=0.3).add_to(m_prev)
