...
# VERDE ANA - SFONDO MASCHERE VERDE + TASTI VERDE ANA + TESTO NERO GRASSETTO TIMES
st.markdown("""
<style>
...
.stButton>button{background:#0e7a3d!important; color:white!important; font-family:"Times New Roman", Times, serif!important; font-weight:bold!important; border:2px solid #0e7a3d!important;}
...
def header():
 st.markdown("<div style='text-align:center;background:#a5d6a7;padding:8px;border-radius:8px;border:2px solid #0e7a3d;'><b style='color:#000000; font-family:\"Times New Roman\", Times, serif; font-size:32px; font-weight:bold;'>VOLONTARIATO<br>Sezione di Varese</b></div>", unsafe_allow_html=True)
...
if st.session_state.map_expand or st.session_state.map_all_expand:
  ...
  if HAS_FOLIUM:
   ...
   for idx in range(len(st.session_state.post)):
    ...
    try:
     ...
     pass
   # NIENTE ICONA DI DEFAULT - SOLO POSTAZIONI SALVATE
 ...
 if scelta=='Dashboard':
  ...
  st.markdown('<p style="font-family:Times New Roman; color:#000000; font-weight:bold;">Clicca sulla mappa dove vuoi mettere la postazione - si aggiorna automatico nella maschera sotto (nessuna icona di default)</p>',unsafe_allow_html=True)
  icon_names=[]
  ...
  if st.button('AGGIUNGI TIPOLOGIA'):
   if new_tipo and new_tipo not in st.session_state.tipologie:
    ...
    st.rerun()
  st.divider()
  st.markdown('##### 1 - MAPPA - CLICCA DOVE VUOI LA POSTAZIONE (SENZA ICONA DEFAULT)')
  c1,c2=st.columns([3,1])
  with c1: st.caption('CLICCA SULLA MAPPA - DECIDI TU - Nessuna icona di default, solo postazioni salvate')
  with c2:
  ...
  if HAS_FOLIUM:
   ...
   for idx in range(len(st.session_state.post)):
    ...
    try:
     ...
     pass
   # TOLTA ICONA DI DEFAULT VERDE - MAPPA PULITA
  ...
  st.divider()
  st.markdown('##### 2 - MASCHERA VERDE ANA CON ODV OPERANTE')
