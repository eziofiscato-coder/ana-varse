...
if st.session_state.map_expand or st.session_state.map_all_expand:
 ...
 if scelta == 'Dashboard':
  ...
  torna()
  st.markdown('#### MAPPA - CON ESPANSIONE SCHERMO INTERO')
  ...
  with st.form('g'):
   ...
   if st.form_submit_button('SALVA POSTAZIONE'):
    if m1:
     ...
     nuovo = {'Postazione': m1, 'Comune': m2, 'Via': m3, 'Lat': m5, 'Lon': m6, 'Tipo': tf, 'Col': ic.get('col', 'blue'), 'Resp': rf}
