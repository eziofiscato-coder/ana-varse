...
.stTextInput>div>div>input{color:#000000!important; font-family:Times New Roman, Times, serif!important; font-weight:bold!important;}
...
def header():
    ...
    with c1:
        try:
            ...
            st.markdown("<div style='font-size:60px; text-align:center;'>ANA</div>", unsafe_allow_html=True)
    with c2:
        st.markdown("<div style='background:#a5d6a7; padding:15px; border-radius:8px; border:2px solid #0e7a3d; text-align:center; margin-top:10px;'><b style='color:#000000; font-family:Times New Roman; font-size:30px;'>VOLONTARIATO<br>Sezione di Varese</b></div>", unsafe_allow_html=True)
...
if st.session_state.map_expand or st.session_state.map_all_expand:
    ...
    if st.session_state.map_expand:
        ...
        if HAS_FOLIUM:
            ...
            if st.session_state.clicked_lat is not None:
                folium.Marker([st.session_state.clicked_lat, st.session_state.clicked_lon], popup='NUOVA', icon=folium.Icon(color='green', icon='ok')).add_to(m)
    ...
    if scelta == 'Dashboard':
        ...
        torna()
        st.markdown('<h3>MAPPA - DECIDI TU POSTAZIONE</h3>', unsafe_allow_html=True)
        icon_names = []
        ...
        c1, c2 = st.columns([3, 1])
        with c1:
            st.caption('CLICCA SULLA MAPPA - Lascia icona verde e compila maschera')
        with c2:
        ...
        with c1:
            st.write('Lat: ' + str(round(st.session_state.lat_tmp, 6)))
        with c2:
            st.write('Comune: ' + st.session_state.com_tmp)
        with c3:
            st.write('Via: ' + st.session_state.via_tmp)
        st.divider()
        st.markdown('##### 2 - MASCHERA INSERIMENTO')
        st.success('Clicca mappa sopra - campi si riempiono da soli')
        vol_nomi = []
