...
.stForm{background:#e8f5e9!important; border:2px solid #2e7d32!important; border-radius:8px!important;}
...
keys = [('dati', []), ('post', []), ('icone', []), ('tipologie', []), ('menu', 'Dashboard'), ('auth', False), ('lat_tmp', 45.8205), ('lon_tmp', 8.8250), ('com_tmp', ''), ('via_tmp', ''), ('map_expand', False), ('map_all_expand', False), ('sel_idx', -1), ('zoom_level', 16)]
...
if st.session_state.map_expand or st.session_state.map_all_expand:
 ...
 if st.session_state.map_expand:
  ...
  if st.button('TORNA INDIETRO', use_container_width=True, type='primary', key='close1'):
   st.session_state.map_expand = False
  ...
  if HAS_FOLIUM:
   ...
   st_folium(m, height=850, width=1600, key='map1fs')

 if st.session_state.map_all_expand:
  st.markdown('## MAPPA TUTTE POSTAZIONI - SCHERMO INTERO 100%')
  if st.button('TORNA INDIETRO', use_container_width=True, type='primary', key='close2'):
   st.session_state.map_all_expand = False
   st.rerun()
  if HAS_FOLIUM and st.session_state.post:
   ...
   for p in st.session_state.post:
    try:
     ...
     pass
   st_folium(m2, height=850, width=1600, key='map2fs')
 ...
 if scelta == 'Dashboard':
  ...
  with c1:
   st.info('Menu completo - tutti i form disponibili')
  with c2:
  ...
  c1, c2 = st.columns([3, 1])
  with c1:
   st.caption('Clicca per collegare Comune Via Lat Lon')
  with c2:
   if st.button('ESPANDI A SCHERMO INTERO', use_container_width=True, key='exp1'):
  ...
  st.divider()
  st.markdown('##### 2 - MASCHERA COLLEGATA + RESPONSABILE')
  ...
  with st.form('g'):
   ...
   with c2:
    m5 = st.text_input('Lat *', value=str(st.session_state.lat_tmp) if st.session_state.com_tmp else '')
    m6 = st.text_input('Lon *', value=str(st.session_state.lon_tmp) if st.session_state.com_tmp else '')
   ...
   if st.form_submit_button('SALVA POSTAZIONE'):
    if m1:
     ...
     nuovo = {'Postazione': m1, 'Comune': m2, 'Via': m3, 'Lat': m5, 'Lon': m6, 'Tipo': tipo_finale, 'Col': ic.get('col', 'blue'), 'Resp': resp_finale}
  ...
  st.divider()
  st.markdown('##### 3 - MAPPA TUTTE LE POSTAZIONI SOTTO LA MASCHERA')
  c1, c2 = st.columns([3, 1])
  with c1:
   st.caption('Totale: ' + str(len(st.session_state.post)))
  with c2:
   if st.button('ESPANDI A SCHERMO INTERO', use_container_width=True, key='exp2'):
