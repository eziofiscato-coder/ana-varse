...
if scelta=='Dashboard':
 ...
 torna()
 st.markdown('#### VOLONTARI - NOME E COGNOME PER RESPONSABILE MAPPA')
 with st.form('vol'):
  st.info('Inserisci Nome e Cognome - poi li trovi in Mappa nel campo Responsabile')
  c1,c2=st.columns(2)
  with c1:
   a1=st.text_input('Nome *', placeholder='Es: Mario')
   a2=st.text_input('Cognome *', placeholder='Es: Rossi')
   a3=st.text_input('Associazione *', value='ANA Varese')
  with c2:
   a4=st.text_input('Cellulare *', placeholder='Es: 3331234567')
  ...
  if st.form_submit_button('SALVA VOLONTARIO'):
   if a1 and a2 and a4:
    ...
    nuovo={'Nome':nome_completo,'Associazione':a3,'Cellulare':a4,'Ruolo':a5}
    st.session_state.dati.append(nuovo)
    save_json(FD,st.session_state.dati)
    st.success(f'Salvato {nome_completo} - ora lo trovi in Mappa > Responsabile')
    st.rerun()
   else:
    st.error('Compila Nome, Cognome e Cellulare *')
 if st.session_state.dati:
  st.divider()
  st.markdown('##### Lista Volontari - agganciati in Mappa')
  st.dataframe(pd.DataFrame(st.session_state.dati), use_container_width=True)

elif scelta=='Mappa':
 torna()
 st.markdown('#### MAPPA - CLICCA E COLLEGA A MASCHERA + RESPONSABILE VOLONTARI')
 ...
 st.divider()
 st.markdown('##### 1 - MAPPA CLICCABILE - CLICCA E COLLEGA A MASCHERA SOTTO')
   ...
   if HAS_FOLIUM:
    ...
    if out and out.get('last_clicked'):
     ...
     if abs(nl-lat_c)>0.000001:
      # COLLEGA AUTOMATICO A MASCHERA - COMUNE VIA LAT LON
 ...
 with col_b:
  st.markdown('**Collegamento automatico**')
  st.write('1. Clicca su mappa OSM')
  st.write('2. Comune, Via, Lat, Lon si collegano sotto')
  st.write('3. Responsabile = lista Volontari')
  st.divider()
 ...
 # Lista volontari per campo responsabile
 volontari_nomi=[d.get('Nome','') for d in st.session_state.dati if d.get('Nome','')]
 if not volontari_nomi:
  st.warning('⚠️ Nessun volontario inserito - vai in Volontari e inserisci Nome e Cognome, poi li trovi qui in Responsabile')
  volontari_nomi=['Nessun volontario - inserisci prima in Volontari']

 with st.form('g'):
  st.info('I campi Comune, Via, Lat, Lon si collegano automatico quando clicchi sulla mappa sopra')
  c1,c2=st.columns(2)
  with c1:
   m1=st.text_input('Nome postazione *', value='', placeholder='Es: Presidio Piazza')
  ...
  with c2:
   m5=st.text_input('Latitudine *', value=str(st.session_state.lat_tmp) if st.session_state.com_tmp else '', placeholder='Auto dal click')
   m6=st.text_input('Longitudine *', value=str(st.session_state.lon_tmp) if st.session_state.com_tmp else '', placeholder='Auto dal click')
   # RESPONSABILE AGGANCIATO A VOLONTARI - NOME E COGNOME
   m8=st.selectbox('Responsabile * (agganciato a Volontari)', volontari_nomi, index=0)
   m8_new=st.text_input('Oppure nuovo responsabile (verra salvato come volontario)', value='')
  ...
  if st.form_submit_button('SALVA POSTAZIONE'):
   if m1:
    ...
    if tipo_finale not in st.session_state.tipologie and tipo_finale:
     ...
     save_json(FT,st.session_state.tipologie)
    # Se nuovo responsabile, salvalo come volontario
    resp_finale=m8_new if m8_new else m8
    if m8_new and m8_new not in volontari_nomi:
     nuovo_vol={'Nome':m8_new,'Associazione':'ANA Varese','Cellulare':'','Ruolo':'Volontario'}
     st.session_state.dati.append(nuovo_vol)
     save_json(FD,st.session_state.dati)
    ...
    nuovo={
     ...
     'Resp':resp_finale,'Note':m10
    }
    st.session_state.post.append(nuovo)
    save_json(FP,st.session_state.post)
    st.success(f'Salvata {m1} - {m2} {m3} - Resp: {resp_finale}')
    st.rerun()

 st.divider()
 st.markdown('##### 3 - MAPPA CON TUTTE LE POSTAZIONI SOTTO LA MASCHERA')

 if st.button('⛶ ESPANDI MAPPA TUTTE POSTAZIONI A TUTTO SCHERMO' if not st.session_state.map_all_expand else '⛶ RIDUCI MAPPA', key='exp2'):
  ...
  if HAS_FOLIUM:
   ...
   for p in st.session_state.post:
    try:
     ...
     fp=p.get('IconaFile','')
     if fp and os.path.exists(fp):
      folium.Marker([la,lo], popup=f"{p.get('Postazione','')} - Resp: {p.get('Resp','')}", tooltip=p.get('Postazione',''), icon=folium.CustomIcon(fp, icon_size=(28,28))).add_to(m2)
     else:
      folium.Marker([la,lo], popup=f"{p.get('Postazione','')} - Resp: {p.get('Resp','')}", tooltip=p.get('Postazione',''), icon=folium.Icon(color=p.get('Col','blue'))).add_to(m2)
    except:
     pass
   st_folium(m2, height=map_all_h, width=map_all_w, key='m2')
  st.success(f"Totale postazioni: {len(st.session_state.post)}")
 else:
  st.info('Nessuna postazione - clicca sulla mappa sopra per iniziare')
 ...
 if st.session_state.post:
  ...
  for i,p in enumerate(st.session_state.post):
   ...
   with c2:
    if st.button('Vedi', key=f'vedi_{i}'):
