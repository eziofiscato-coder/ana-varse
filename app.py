...
# CAMPI NERO GRASSETTO TIMES + MASCHERE VERDE ANA + TASTI VERDE ANA
st.markdown("""
<style>
...
.stForm label{color:#000000!important; font-family:"Times New Roman", Times, serif!important; font-weight:bold!important; font-size:16px!important;}
.stTextInput>div>div>input{color:#000000!important; font-family:"Times New Roman", Times, serif!important; font-weight:bold!important; font-size:16px!important;}
.stSelectbox>div>div{color:#000000!important; font-family:"Times New Roman", Times, serif!important; font-weight:bold!important;}
...
if 'preview_idx' not in st.session_state: st.session_state.preview_idx=-1
...
def header():
 st.markdown("<div style='margin-top:10px; margin-bottom:30px;'></div>", unsafe_allow_html=True)
 ...
 with col2:
  st.markdown("<div style='background:#a5d6a7; padding:15px; border-radius:8px; border:2px solid #0e7a3d; text-align:center; margin-top:10px;'><b style='color:#000000; font-family:\"Times New Roman\", Times, serif; font-size:30px; font-weight:bold;'>VOLONTARIATO<br>Sezione di Varese</b></div>", unsafe_allow_html=True)
...
if st.session_state.map_expand or st.session_state.map_all_expand:
 ...
 if st.session_state.map_expand:
  ...
  if HAS_FOLIUM:
   ...
   if st.session_state.clicked_lat is not None:
    folium.Marker([st.session_state.clicked_lat, st.session_state.clicked_lon], popup='NUOVA POSIZIONE', icon=folium.Icon(color='green', icon='ok')).add_to(m)
 ...
 if scelta=='Dashboard':
  ...
  st.markdown('<p style="font-family:Times New Roman; color:#000000; font-weight:bold;">Clicca sulla mappa - lascia icona verde e compila automatico Comune/Via/Lat/Lon</p>',unsafe_allow_html=True)
  icon_names=[]
  ...
  c1,c2=st.columns([3,1])
  with c1: st.caption('CLICCA SULLA MAPPA - Lascia icona verde e associa maschera')
  with c2:
  ...
  if HAS_FOLIUM:
   ...
   for idx in range(len(st.session_state.post)):
    ...
    try:
     ...
     pass
   if st.session_state.clicked_lat is not None:
    folium.Marker([st.session_state.clicked_lat, st.session_state.clicked_lon], popup='NUOVA - Compila maschera', icon=folium.Icon(color='green', icon='ok')).add_to(m)
  ...
  st.divider()
  st.markdown('##### 2 - MASCHERA VERDE ANA - VIA COMUNE LAT LON + ODV - CAMPI NERO GRASSETTO')
  st.success('Clicca mappa sopra - si riempiono automatico Comune/Via/Lat/Lon + icona verde')
  vol_nomi=[]
  ...
  with st.form('g'):
   ...
   if st.form_submit_button('SALVA POSTAZIONE CON ICONA'):
    if m1:
     ...
     st.success('Salvata '+m1+' - ICONA INSERITA!')
     st.rerun()
  st.divider()
  ...
  st.divider()
  st.markdown('##### 4 - ELENCO POSTAZIONI - ANTEPRIMA INGRANDITA SENZA VELO ROSSO')
  if st.session_state.post:
   st.dataframe(pd.DataFrame(st.session_state.post),use_container_width=True)
   if st.session_state.preview_idx>=0 and st.session_state.preview_idx<len(st.session_state.post):
    p_prev=st.session_state.post[st.session_state.preview_idx]
    st.markdown('#### ANTEPRIMA MAPPA - POSTAZIONE SELEZIONATA - INGRANDIMENTO SENZA VELO')
    st.success('Anteprima: '+p_prev.get('Postazione','')+' - '+p_prev.get('Comune','')+' - '+p_prev.get('Via',''))
    try:
     ...
     if HAS_FOLIUM:
      ...
      folium.Marker([la_prev,lo_prev],popup=p_prev.get('Postazione',''),icon=folium.Icon(color='red',icon='star')).add_to(m_prev)
      # TOLTO VELO ROSSO - NIENTE CIRCLE
      st_folium(m_prev,height=350,width=700,key='m_preview')
    except:
     pass
   for i in range(len(st.session_state.post)):
    ...
    with c2:
     if st.button('Vedi in mappa grande',key='vedi_'+str(i)):
      ...
      st.rerun()
    with c3:
     if st.button('Anteprima qui sotto',key='ante_'+str(i)):
      st.session_state.preview_idx=i
      st.rerun()
    with c4:
     lat=p.get('Lat','')
