...
if st.session_state.exp1 or st.session_state.exp2 or st.session_state.exp3:
 ...
 if scelta=='Dashboard':
  ...
  st.caption('Clicca mappa - inserisce Comune/Via/Lat/Lon esatti')
  ...
  st.divider()
  st.markdown('##### 1 - MAPPA - CLICCA DOVE VUOI - COMUNE/VIA ESATTI')
   ...
   if st.session_state.clat is not None:
    try:
     ...
     if ftmp and os.path.exists(ftmp):
      ...
      folium.Marker([st.session_state.clat,st.session_state.clon],popup='NUOVA CON ICONA PNG',icon=ic).add_to(m)
     else:
  ...
  st.divider()
  st.markdown('##### 2 - MASCHERA - COMUNE VIA LAT LON + RESPONSABILE')
  ...
  if not vol_nomi:
   vol_nomi=['Nessun volontario - vai in Volontari']
  ...
  with st.form('form_mappa'):
   ...
   if st.form_submit_button('SALVA POSTAZIONE CON ICONA PNG'):
    if m1:
     ...
     st.success(f"Salvata {m1} a {m2} {m3}")
     st.rerun()
  ...
  st.divider()
  st.markdown('##### 4 - ELENCO POSTAZIONI CON ICONA PNG E COMUNE VIA')
  ...
  if st.session_state.post:
   ...
   if st.session_state.prev>=0:
    try:
     ...
     st.info(f"Comune: {com} - Via: {via} - Lat: {lat} - Lon: {lon}")
     st.info(f"ODV: {odv} - Resp: {resp}")
  ...
  if st.session_state.cons:
   st.dataframe(pd.DataFrame(st.session_state.cons),use_container_width=True)
