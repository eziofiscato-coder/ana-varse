...
for k,v in [("dati",[]),("postazioni",[]),("icone_lib",[]),("menu_scelta","Dashboard"),("map_lat",45.8205),("map_lon",8.8250),("map_via",""),("map_comune","Varese"),("map_logo","Default"),("post_sel",None),("map_zoom",13),("authenticated",False)]:
...
if scelta=="Dashboard":
    ...
    torna_dashboard()
    st.markdown("## 📍 MAPPA POSTAZIONI - FIX ICONE ANTEPRIMA + VIA ASSOCIATA")
    ...
    with c1:
        ...
        try:
            ...
            if map_data and map_data.get("last_clicked"):
                ...
                st.toast(f"Via rilevata: {st.session_state.map_via}")
                st.rerun()
        except Exception as e:
            st.error(f"Errore: {e}")

        st.markdown("### 🔍 ANTEPRIMA - TUTTE LE POSTAZIONI CON ICONE")
        tipo_mappa=st.selectbox("Tipo mappa", ["StreetMap","OpenTopoMap (OTM)","Google Maps Stradale","Google Satellite","Waze Style"], index=0, key="sel_tipo_anteprima")
        ...
        try:
            ...
            for idx_p, p in enumerate(st.session_state.postazioni):
                try:
                    ...
                    via_p=p.get('Via','')
                    b64=trova_b64_logo(logo_nome, st.session_state.icone_lib)
                    popup_text=f"{nome_post} - {via_p} - {logo_nome}"
                    if b64:
                        ...
                        if tmp_path:
                            ...
                            folium.Marker([lat_f, lon_f], popup=popup_text, icon=folium.Icon(color="blue", icon="ok")).add_to(m2)
                    else:
                        folium.Marker([lat_f, lon_f], popup=popup_text, icon=folium.Icon(color="red", icon="flag")).add_to(m2)
                except:
                    pass

            st_folium(m2,width=1000,height=650,key="mappa_anteprima_tutte")
            if st.session_state.postazioni:
                st.success(f"✅ ANTEPRIMA {tipo_mappa} - {len(st.session_state.postazioni)} POSTAZIONI CON ICONE 15x15 VISIBILI")
            else:
    ...
    with c2:
        ...
        with st.form("form_post_finale"):
            ...
            via_post=st.text_input("Via * - Via selezionata",value=val_via,key="inp_via")
            ...
            salva_post=st.form_submit_button("📍 SALVA",use_container_width=True,type="primary")
            if salva_post:
                if nome_post and lat_post and lon_post:
                    ...
                    st.success(f"Salvata {nome_post} - Via {via_post}")
                    st.rerun()

    st.divider()
    st.markdown("### 📋 ELENCO POSTAZIONI - Clicca per associare Via in maschera + Zoom anteprima")
    if st.session_state.postazioni:
