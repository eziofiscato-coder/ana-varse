...
if scelta=="Dashboard":
    ...
    if st.session_state.volontario_selezionato is not None:
        ...
        with t1:
            ...
            st.markdown('</div>',unsafe_allow_html=True)
        with t2:
            st.markdown('<div class="submask"><h4>Radio</h4></div>',unsafe_allow_html=True)
        with t3:
            st.markdown('<div class="submask"><h4>Eventi</h4></div>',unsafe_allow_html=True)
        with t4:
            st.markdown('<div class="submask">',unsafe_allow_html=True)
            ...
            with st.form("form_pres"):
                ...
                salva_pres=st.form_submit_button("Registra presenza",use_container_width=True)
                if salva_pres:
                    st.success(f"{ore}h a {luogo}")
            st.markdown('</div>',unsafe_allow_html=True)
        with t5:
            st.markdown('<div class="submask"><h4>Emergenze</h4></div>',unsafe_allow_html=True)
        with t6:
            st.markdown('<div class="submask">',unsafe_allow_html=True)
            st.markdown("#### Note")
    ...
    with st.form("form"):
        ...
        if salva_vol:
            if nome and cognome and cell and assoc:
                ...
                st.rerun()
    st.divider()
    st.markdown("### Elenco - Seleziona per dettaglio")
    ...
    st.info("Carica qui i loghi PNG - Clicca sul logo salvato per vederlo su Google / OSM / Waze")
    ...
    if st.session_state.icone_lib:
        st.markdown("#### Libreria - Seleziona per vedere su mappa")
        cols=st.columns(4)
    ...
    with c1:
        st.markdown("### Mappa con loghi scelti")
        ...
        if HAS_FOLIUM:
            ...
            for idx_p, p in enumerate(st.session_state.postazioni):
                try:
                    ...
                    pass
            folium.Marker([st.session_state.map_lat, st.session_state.map_lon], popup="Nuova", icon=folium.Icon(color="red", icon="plus")).add_to(m)
            ...
            if map_data and map_data.get("last_clicked"):
                ...
                st.rerun()
        st.markdown("#### Apri su Google Maps / OSM / Waze")
        lat=st.session_state.map_lat
        lon=st.session_state.map_lon
        if st.session_state.postazione_selezionata_per_mappa:
            p_sel=st.session_state.postazione_selezionata_per_mappa
            lat_sel=p_sel.get('Latitudine',lat)
            lon_sel=p_sel.get('Longitudine',lon)
            logo_sel=p_sel.get('Icona','📍')
            c_osm,c_gm,c_waze=st.columns(3)
            with c_osm:
                st.link_button(f"OSM - {logo_sel}", f"https://www.openstreetmap.org/?mlat={lat_sel}&mlon={lon_sel}#map=17/{lat_sel}/{lon_sel}", use_container_width=True)
            with c_gm:
                st.link_button(f"Google - {logo_sel}", f"https://www.google.com/maps/search/?api=1&query={lat_sel},{lon_sel}", use_container_width=True)
            with c_waze:
                st.link_button(f"Waze - {logo_sel}", f"https://waze.com/ul?ll={lat_sel},{lon_sel}&navigate=yes", use_container_width=True)
        else:
            c_osm,c_gm,c_waze=st.columns(3)
            with c_osm:
                st.link_button("OSM", f"https://www.openstreetmap.org/?mlat={lat}&mlon={lon}#map=17/{lat}/{lon}", use_container_width=True)
            with c_gm:
    ...
    with c2:
        ...
        st.markdown(f"**Logo selezionato:** {st.session_state.map_logo_selezionato}")
        b64_sel=trova_b64_logo(st.session_state.map_logo_selezionato, st.session_state.icone_lib)
        if b64_sel:
            st.image(f"data:image/png;base64,{b64_sel}",width=120,caption=f"{st.session_state.map_logo_selezionato}")
        ...
        with st.form("form_post"):
            ...
            salva_post=st.form_submit_button("Salva postazione",use_container_width=True,type="primary")
            if salva_post:
                if nome_post and lat_post and lon_post:
                    ...
                    st.success(f"{st.session_state.map_logo_selezionato} {nome_post} salvata!")
                    st.rerun()
    st.divider()
        ...
        for idx, p in enumerate(st.session_state.postazioni):
            ...
            with c_img:
                ...
                if b64:
                    ...
                    if st.button(f"Vedi",key=f"view_img_{idx}"):
                    ...
                    st.markdown(f"<div style='font-size:30px;'>{logo_nome}</div>",unsafe_allow_html=True)
            with c1:
                st.write(f"{p.get('Postazione','')}")
            with c2:
                st.write(f"{p.get('Comune','')}")
            with c3:
                if st.button(f"Seleziona per mappa",key=f"use_em_{idx}",use_container_width=True):
                    st.session_state.postazione_selezionata_per_mappa=p
                    st.session_state.map_lat=float(p.get('Latitudine',45.8205))
                    st.session_state.map_lon=float(p.get('Longitudine',8.8250))
                    st.session_state.map_logo_selezionato=p.get('Icona','📍')
                    st.rerun()
    ...
    c_up1,c_up2=st.columns([2,1])
    with c_up1:
        up_em=st.file_uploader("Carica logo PNG", type=["png","jpg","jpeg"], key="up_icon_em")
        if up_em:
            b64=img_to_b64(up_em)
            nome_em=st.text_input("Nome logo", value=up_em.name.split(".")[0], key="nome_icona_em")
            if st.button("Salva logo",key="save_icon_em",use_container_width=True,type="primary"):
        ...
        for i, ic in enumerate(st.session_state.icone_lib):
            with cols[i%4]:
                ...
                if ic.get("b64"):
                    st.image(f"data:image/png;base64,{ic['b64']}",width=80)
                    st.write(f"{ic['nome']}")
                    if st.button(f"Seleziona",key=f"view_em_{i}",use_container_width=True):
                        st.session_state.em_logo_selezionato=ic['nome']
                        st.rerun()
                else:
                    st.markdown(f"<div style='font-size:40px;'>{ic.get('emoji',ic.get('nome','🚨'))}</div>",unsafe_allow_html=True)
                if st.button(f"Elimina",key=f"del_em_{i}",use_container_width=True):
                    st.session_state.icone_lib.pop(i)
                    save_json(FILE_ICONE_COND,st.session_state.icone_lib)
                    st.rerun()
                st.markdown('</div>',unsafe_allow_html=True)
    ...
    idx_em=0
    if st.session_state.em_logo_selezionato in opzioni_logo_em:
        idx_em=opzioni_logo_em.index(st.session_state.em_logo_selezionato)
    sel_logo_em=st.selectbox("Scegli logo", opzioni_logo_em, index=idx_em, key="sel_logo_em_combo")
    if sel_logo_em!=st.session_state.em_logo_selezionato:
        st.session_state.em_logo_selezionato=sel_logo_em
        st.rerun()
    with st.form("form_em",clear_on_submit=False):
        ...
        with c2:
            odv_int=st.selectbox("ODV *",["ANA Varese","Protezione Civile","Croce Rossa"])
        azione_int=st.text_area("Azione *",height=100)
        salva_em=st.form_submit_button("Salva emergenza",use_container_width=True,type="primary")
        if salva_em:
            if comune_int and via_int and azione_int:
                st.session_state.interventi_lista.append({"Comune":comune_int,"Via":via_int,"ODV Operativa":odv_int,"Azione":azione_int,"Logo":st.session_state.em_logo_selezionato})
    ...
    if "backup_unico" in st.session_state:
        st.download_button("Scarica Excel",st.session_state["backup_unico"],file_name=f"BACKUP_{date.today()}.xlsx",mime=MIME,use_container_width=True)