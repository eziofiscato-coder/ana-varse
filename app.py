...
if scelta=='Dashboard':
 ...
 torna()
 st.markdown('#### MAPPA - TIPO MAPPA + ESPANSIONE + TABELLA CLICCABILE')

 tipo_map=st.radio('Tipo mappa', ['OpenStreetMap','Google Maps','Google Earth','Waze'], horizontal=True)
 if st.button('ESPANDI MAPPA' if not st.session_state.map_expand else 'RIDUCI MAPPA'):
  ...
  if tipo_map=='OpenStreetMap':
   if HAS_FOLIUM:
    ...
    if fp and os.path.exists(fp):
     folium.Marker([lat_c,lon_c], popup='Nuova', icon=folium.CustomIcon(fp, icon_size=(32,32))).add_to(m)
    else:
     folium.Marker([lat_c,lon_c], popup='Nuova', icon=folium.Icon(color=ic.get('col','green'), icon='plus')).add_to(m)
 ...
 st.divider()
 st.markdown('##### MASCHERA - AGGANCIA AUTOMATICO')
 with st.form('g'):
  c1,c2=st.columns(2)
  with c1:
 ...
 st.divider()
 st.markdown('##### MAPPA CON TUTTE LE POSTAZIONI')
 ...
 st.divider()
 st.markdown('##### TABELLA CLICCABILE - COME STAMATTINA')
 if st.session_state.post:
