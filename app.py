...
st.markdown("<style>.block-container{padding-top:8px!important; max-width:100%!important;}.stForm{background:#e8f5e9!important; border:2px solid #000000!important;}.stButton>button{background:#000000!important; color:white!important; font-family:\"Times New Roman\"; font-weight:bold;}</style>", unsafe_allow_html=True)
...
def header():
 st.markdown("<div style='text-align:center;background:#ffffff;padding:8px;border-radius:8px;border:2px solid #000000;'><b style='color:#000000; font-family:\"Times New Roman\", Times, serif; font-size:32px; font-weight:bold;'>VOLONTARIATO<br>Sezione di Varese</b></div>", unsafe_allow_html=True)
...
if st.session_state.map_expand or st.session_state.map_all_expand:
  ...
  if HAS_FOLIUM:
   ...
   for idx in range(len(st.session_state.post)):
    ...
    try:
     ...
     pass
   folium.Marker([lat_c,lon_c],popup='TU SEI QUI - Decidi tu la postazione',icon=folium.Icon(color='green')).add_to(m)
 ...
 if scelta=='Dashboard':
  st.markdown('<h3 style="font-family:Times New Roman; color:#000000; font-weight:bold;">DASHBOARD - MENU COMPLETO</h3>',unsafe_allow_html=True)
  ...
  st.markdown('<p style="font-family:Times New Roman; color:#000000; font-weight:bold;">Clicca sulla mappa dove vuoi mettere la postazione - si aggiorna automatico nella maschera sotto</p>',unsafe_allow_html=True)
  icon_names=[]
  ...
  if st.button('AGGIUNGI TIPOLOGIA'):
   if new_tipo and new_tipo not in st.session_state.tipologie:
    ...
    st.rerun()
  st.divider()
  st.markdown('##### 1 - MAPPA - CLICCA DOVE VUOI LA POSTAZIONE')
  c1,c2=st.columns([3,1])
  with c1: st.caption('CLICCA SULLA MAPPA - DECIDI TU - Si aggiorna nella maschera sotto')
  with c2:
  ...
  if HAS_FOLIUM:
   ...
   for idx in range(len(st.session_state.post)):
    ...
    try:
     ...
     pass
   folium.Marker([lat_c,lon_c],popup='TU DECIDI QUI',icon=folium.Icon(color='green')).add_to(m)
  ...
  st.divider()
  st.markdown('##### 2 - MASCHERA - CON ODV OPERANTE - DECIDI TU')
  st.info('Campi si aggiornano quando clicchi mappa - poi decidi tu nome e salvi')
  vol_nomi=[]
  ...
  with st.form('g'):
   ...
   if st.form_submit_button('SALVA POSTAZIONE CON ICONA'):
    if m1:
     ...
     st.success('Salvata '+m1+' ODV:'+odv_f+' - ICONA INSERITA IN MAPPA!')
     st.rerun()
  st.divider()
