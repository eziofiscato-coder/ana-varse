...
def hdr():
    ...
    with c1:
        try:
            ...
            st.markdown('**ANA**')
    with c2:
        st.markdown(f'<div style="background:{VERDE};padding:12px;border-radius:8px;color:white;font-weight:bold;font-size:18px;">NUCLEO PROT CIVILE ANA VARESE</div>', unsafe_allow_html=True)
...
if st.session_state.page == 'entra':
    ...
    if cur == 'Dashboard':
        ...
        with st.form('alias_form'):
            ...
            if st.button('Salva Alias Radio', type='primary', use_container_width=True):
                pass
            ...
            with st.form('form_int'):
                ...
                if st.form_submit_button('SALVA INTERVENTO COLLEGATO A EMERGENZA BLINDATA', use_container_width=True, type='primary'):
                    if comune_int and via_int and azione_int:
                        ...
                        st.success('Intervento salvato collegato a Emergenza')
