...
if sc=='Dashboard':
    st.markdown("## Dashboard ANA - 1130 righe")
    a,b=st.columns(2)
    with a:
    ...
    to_dash()
    st.markdown("## Volontari - Maschera completa")
    if st.session_state.edit_idx >=0:
        ...
        with st.form("edit_vol"):
            ...
            if ba:
                st.session_state.edit_idx=-1
                st.rerun()
    st.markdown("#### MASCHERA PRINCIPALE - ANAGRAFICA VISIBILE")
    ...
    with st.expander("SOTTOMASCHERA 2 - FOTO"):
        vlist=[d.get('Nome','') for d in st.session_state.dati]
        if vlist:
            sel=st.selectbox("Volontario",vlist,key='c3')
            idx=-1
            for i,d in enumerate(st.session_state.dati):
                if d.get('Nome','')==sel:
                    idx=i
            ...
            if idx>=0:
                ...
                if up:
                    ...
                    if st.button("SALVA FOTO"):
                        ...
                        st.success("OK foto")
                        st.rerun()
    with st.expander("SOTTOMASCHERA 3 - TESSERINO 860x540"):
        vlist=[d.get('Nome','') for d in st.session_state.dati]
        if vlist:
            sel=st.selectbox("Volontario",vlist,key='c4')
            idx=-1
            for i,d in enumerate(st.session_state.dati):
                if d.get('Nome','')==sel:
                    idx=i
            ...
            if idx>=0:
                ...
                if tess:
                    st.image(tess)
                    st.download_button('SCARICA TESSERINO',tess,file_name=f"Tess_{vol.get('Nome','')}.png",mime='image/png',key='tess_dl')
    ...
    to_dash()
    st.markdown("## Mappa Postazioni - Fullscreen + Icona da libreria")
    c1,c2=st.columns(2)
    with c1:
    ...
    if HAS_MAP:
        try:
            ...
            Fullscreen(position='topleft',title='Tutto schermo',title_cancel='Esci',force_separate_button=True).add_to(mm)
            ...
            if mp and mp.get('last_clicked'):
                ...
                st.success(f"Pos cliccata - Icona {st.session_state.sel_icon}")
    ...
    with st.form("icone",clear_on_submit=True):
        ...
        bi=st.form_submit_button("SALVA ICONA IN LIBRERIA")
        if bi:
            if i_nome:
                fp=""
                if i_file:
    ...
    with st.form("emerg",clear_on_submit=True):
        ...
        az_int=st.text_area("Azione *",height=100)
    ...
    to_dash()
    st.markdown("## CHAT VOLONTARI COLLEGATI")
    st.markdown("### Form chat con altri volontari - Ripristinata!")
    st.markdown("### Come WhatsApp - Volontari collegati chattano tra loro")
    ...
    if st.session_state.chat:
        for msg in st.session_state.chat[-20:]:
            ...
            if user==st.session_state.chat_user:
                ...
                st.markdown(f"<div style='background:#f0f0f0;padding:8px;border-radius:8px;margin:5px;'><b>{user}</b> {time}<br>{text}</div>", unsafe_allow_html=True)
    else:
        st.info("Nessun messaggio - Inizia tu")
    st.divider()
    with st.form("chat_form",clear_on_submit=True):
        chat_text=st.text_input("Scrivi messaggio chat volontari collegati")
        ...
        if chat_send:
            if chat_text:
                ...
                st.rerun()
    with st.expander("Archivio chat"):
        if st.session_state.chat:
