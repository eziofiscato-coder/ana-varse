...
.stButton>button{background:#0e7a3d!important;color:white!important;font-weight:bold!important;font-size:18px!important;}
</style>
...
if st.session_state.popup_cfg.get('mostra',True) and not st.session_state.popup_shown:
 ...
 with c2:
  # Cerca immagine popup
  img_found=False
  for img_name in ['copertina.jpg','copertina.png','benvenuto.jpg','benvenuto.png','popup.jpg','Cartoon_Civil_Protection_Operator','logo.png']:
   if os.path.exists(img_name):
    try:
     ...
     pass
  # Se non trova, mostra placeholder
  if not img_found:
   st.info("Carica la tua immagine come copertina.jpg su GitHub")
   try:
  ...
  st.divider()
  # TASTO ENTRA NEL SISTEMA FIXATO
  if st.button("ENTRA NEL SISTEMA", type="primary", use_container_width=True, key="entra_sistema"):
   st.session_state.popup_shown=True
   st.rerun()
  st.caption("Clicca ENTRA NEL SISTEMA per andare al login")
 st.stop()

# DOPO POPUP - LOGIN
if not st.session_state.auth:
 header()
 # Mostra di nuovo copertina piccola sopra login
 c1,c2,c3=st.columns([1,2,1])
 with c2:
...
if st.session_state.exp1 or st.session_state.exp2 or st.session_state.exp3:
 ...
 if scelta=='Dashboard':
   ...
   with c2:
    if st.button('CREA PDF VOLONTARI',key='pdf_vol_btn'):
     ...
     if pdf:
      ...
      st.success('PDF creato correttamente!')
  ...
  with st.form('form_emerg'):
   ...
   if st.form_submit_button('SALVA EMERGENZA'):
    if e1:
     ...
     st.success(f"Salvata {e1} del {fmt_date(e_data)}")
     st.rerun()
  ...
  torna()
  st.markdown('#### BACKUP - IMPORT/EXPORT EXCEL + PDF')
  st.divider()
  if st.button('CREA BACKUP COMPLETO EXCEL'):
   out=BytesIO()
   with pd.ExcelWriter(out,engine='openpyxl') as writer:
    if st.session_state.dati:
  ...
  if 'bk_all' in st.session_state:
   ...
   if st.button('CREA PDF BACKUP COMPLETO',use_container_width=True):
    df_all=pd.DataFrame([{'Form':'Volontari','Num':len(st.session_state.dati)},{'Form':'Mappa','Num':len(st.session_state.post)},{'Form':'Emergenze','Num':len(st.session_state.emerg)},{'Form':'CheckIn','Num':len(st.session_state.check)},{'Form':'DBRadio','Num':len(st.session_state.radio)},{'Form':'ConsegnaRadio','Num':len(st.session_state.cons)}])
   ...
   if 'pdf_bk_all' in st.session_state:
    st.download_button('SCARICA PDF BACKUP COMPLETO',st.session_state['pdf_bk_all'],file_name='BACKUP_COMPLETO.pdf',mime='application/pdf',use_container_width=True)

 elif scelta=='Impostazioni Popup':
  torna()
  st.markdown('#### IMPOSTAZIONI PAGINA INIZIALE POPUP')
  st.info("Popup prima del login - con tua immagine")
