...
# ===== PASSWORD INIZIALE - RIPRISTINATA =====
...
.stButton>button{background-color:#d32f2f!important;color:white!important;border:2px solid #b71c1c!important;font-weight:bold!important;border-radius:8px!important;}
...
def header_3_loghi_fisso():
    ...
    try:
        ...
        pass
    try:
        if __import__("os").path.exists("logo2.png"):
            c2.image("logo2.png", width=80)
    except:
        pass
    try:
        if __import__("os").path.exists("logo_pc_lombardia.png"):
            c3.image("logo_pc_lombardia.png", width=80)
    except:
        c3.markdown("**Protezione Civile**")
...
def pagina_interventi_emergenza():
    ...
    if st.session_state.interventi_lista:
        ...
        st.download_button("📥 Scarica CSV", df.to_csv(index=False).encode('utf-8'), "interventi_emergenza.csv", "text/csv", use_container_width=True)
    else:
        st.info("Nessun intervento emergenza registrato")

# ===== PAGINA INIZIALE RIPRISTINATA CON 3 TAB =====
# PRIMA PAGINA = Da Anagrafica Esistente (come prima)
tab1, tab2, tab3 = st.tabs(["📚 Da Anagrafica Esistente", "✏️ Inserimento Manuale", "🚨 INTERVENTI EMERGENZA"])

with tab1:
    st.markdown("### 📚 Da Anagrafica Esistente")
    st.markdown("**PRIMA PAGINA RIPRISTINATA COME PRIMA**")
    st.info("Qui va il tuo form anagrafica originale - ora è di nuovo la prima pagina!")
    # Esempio placeholder anagrafica
    if st.session_state.interventi_lista:
        st.markdown("#### Ultimi interventi inseriti:")
        st.dataframe(pd.DataFrame(st.session_state.interventi_lista).tail(3))

with tab2:
    st.markdown("### ✏️ Inserimento Manuale")
    st.info("Form inserimento manuale originale")

with tab3:
    pagina_interventi_emergenza()

st.divider()
st.caption("ANA Varese - Password: ANA2025 | Verde Chiaro + Tasti Rossi | 3 Form")
