...
if sc=='Dashboard':
    ...
    st.markdown("### Maschera come prima")
    if st.session_state.edit_idx >=0:
        ...
        with st.form("edit_vol"):
            ...
            if ba:
                st.session_state.edit_idx=-1
                st.rerun()
    # FIX RIGA 350 - a_tel non troncata - righe corte
    st.markdown("#### MASCHERA 1 - VISIBILE")
    ...
    to_dash()
    st.markdown("## Mappa")
    st.markdown("### Click per icona - Da libreria")
    st.markdown("### Fullscreen e ritorno")
    c1,c2=st.columns(2)
    with c1:
        mt=st.selectbox("Tipo mappa",['Standard','Google','Satellite','Terreno','Waze Chiaro','Waze Scuro'],index=0,key='mt')
        st.session_state.map_type=mt
    with c2:
        ...
        try:
            ...
            for p in st.session_state.post:
                try:
                    ...
                    pass
            if st.session_state.full=='Tutto schermo':
                h=800
                w=1400
            else:
                h=500
                w=1200
            mp=st_folium(mm,width=w,height=h)
