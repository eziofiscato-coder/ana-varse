...
ICONS = {
 ...
 "protezione_civile": {"nome":"Prot. Civile","color":"darkblue","forma":"esagono","icon":"???"},
...
for k,v in [("authenticated",False),("emergenze_lista",[]),("interventi_lista",[]),("eventi",[]),("dati",[]),("mem_nomi",["Mario Rossi","Luigi Bianchi","Giuseppe Verdi"]),("postazioni",[]),("menu_scelta","?? Dashboard")]:
...
with st.sidebar:
    st.markdown(LOGHI, unsafe_allow_html=True)
    opzioni=["?? Dashboard","?? Emergenze con Loghi","??? Mappa Postazioni","?? Interventi Emergenza","?? Gestione Eventi","?? Volontari","?? Radio","?? Backup"]
    idx = opzioni.index(st.session_state.menu_scelta) if st.session_state.menu_scelta in opzioni else 0
    sel=st.radio("MENU", opzioni, index=idx, key="menu_radio_fixed")
...
if scelta=="?? Dashboard":
    ...
    st.markdown("### ? MENU SCELTA RAPIDA - TUTTI I TASTI ORA FUNZIONANO")
    ...
    for i,(k,v) in enumerate(ICONS.items()):
        with cols[i%6]:
            st.markdown(f"<div style='background:white; border:2px solid #2e7d32; border-radius:10px; padding:8px; text-align:center; margin-bottom:8px;'><div style='font-size:28px;'>{v['icon']}</div><div style='font-size:10px; color:#1b5e20;'>{v['nome']}</div><div style='font-size:8px;'>{v['forma']}</div></div>", unsafe_allow_html=True)
    st.divider()
    with st.form("form_emergenza_loghi", clear_on_submit=True):
        ...
        with c3:
            ...
            st.markdown(f"<div style='background:#c8e6c9; border:3px solid #2e7d32; border-radius:12px; padding:15px; text-align:center;'><div style='font-size:50px;'>{icona_sel}</div><div style='color:#1b5e20; font-weight:bold;'>{ICONS[tipo_key]['nome']}</div></div>", unsafe_allow_html=True)
        ...
        if st.form_submit_button("?? SALVA EMERGENZA CON LOGO", use_container_width=True, type="primary"):
            if comune and via and descrizione:
                st.session_state.emergenze_lista.append({"Data": str(data_em), "Ora": str(ora_em), "Logo": ICONS[tipo_key]['icon'], "Tipo": ICONS[tipo_key]['nome'], "Forma": ICONS[tipo_key]['forma'], "Colore": ICONS[tipo_key]['color'], "Comune": comune, "Via": via, "Civico": civico, "Priorità": priorita, "ODV": odv, "Squadre": squadre, "Descrizione": descrizione, "Note": note})
    ...
    if st.session_state.emergenze_lista:
        ...
        st.markdown(f"### ?? Tabella Emergenze - {len(df)} con colonna Logo")
    ...
    st.markdown(LOGHI, unsafe_allow_html=True)
    st.markdown("<h2 style='color:#1b5e20; background:#a5d6a7; padding:12px; border-radius:12px; border:3px solid #2e7d32;'>??? Mappa Postazioni - Vecchio Form Completo</h2>", unsafe_allow_html=True)
    torna_dashboard()
    ...
    if st.session_state.postazioni:
        ...
        try:
            ...
            if not dfm.empty: st.map(dfm[["lat","lon"]], zoom=11, use_container_width=True)
        except: pass
    ...
    st.markdown(LOGHI, unsafe_allow_html=True)
    st.markdown("<h2 style='color:#1b5e20; background:#a5d6a7; padding:12px; border-radius:12px; border:3px solid #2e7d32;'>?? Interventi Emergenza - Vecchio Form Completo con Tutti i Campi</h2>", unsafe_allow_html=True)
    torna_dashboard()
    with st.form("form_emergenza_vecchio", clear_on_submit=True):
        ...
        with c4:
            ...
            mezzi_int=st.text_input("Mezzi Impiegati", placeholder="Fuoristrada 1, Motosega...")
        with c5:
            ref_int=st.text_input("Referente sul Posto")
            tel_ref=st.text_input("Telefono Referente")
        azione_int=st.text_area("Azione / Descrizione Intervento *", height=120, placeholder="Descrivi dettagliatamente l'intervento...")
        note_int=st.text_area("Note / Esito", height=80, placeholder="Esito, danni, note...")
        if st.form_submit_button("?? SALVA INTERVENTO EMERGENZA", use_container_width=True, type="primary"):
            if comune_int and via_int and azione_int:
                ...
                st.success("Intervento Salvato!"); st.balloons(); st.rerun()
    if st.session_state.interventi_lista:
        st.dataframe(pd.DataFrame(st.session_state.interventi_lista), use_container_width=True)
        st.download_button("?? Scarica CSV", pd.DataFrame(st.session_state.interventi_lista).to_csv(index=False).encode('utf-8'), "interventi.csv", use_container_width=True)
    torna_dashboard()
    ...
    st.markdown(LOGHI, unsafe_allow_html=True)
    torna_dashboard()
    st.info(f"Sezione {scelta} - Form in sviluppo")
    torna_dashboard()