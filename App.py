...
# HEADER BASSO - INTESTAZIONI BASSE
st.markdown("""
<style>
...
def header():
 # INTESTAZIONE SEMPLICE - NO GLITCH
 st.markdown(
  "<div style='text-align:center;background:#a5d6a7;padding:5px;border-radius:8px;border:2px solid #2e7d32;'><b style='color:#0e7a3d;'>VOLONTARIATO Varese - ANA</b></div>",
  unsafe_allow_html=True
 )
...
if scelta=="Dashboard":
 st.markdown("#### DASHBOARD - INTESTAZIONE BASSA")
 ...
 torna()
 st.markdown("#### VOLONTARI - INTESTAZIONI CON DESCRIZIONE")
 ...
 with t1:
  with st.form("f1"):
   st.markdown("**Anagrafica - Dati personali**")
 ...
 with t2:
  with st.form("f2"):
   st.markdown("**Residenza - Contatti**")
 ...
 with t3:
  with st.form("f3"):
   st.markdown("**Tesseramento - Ruolo**")
 ...
 with t5:
  with st.form("f5"):
   st.markdown("**Note - Salvataggio finale**")
 ...
 torna()
 st.markdown("#### MAPPA - OTM / OSM + INTESTAZIONI")

 # SELEZIONE OTM OSM GOOGLE WAZE
 st.markdown("**Seleziona mappa OTM**")
 tipo=st.radio(
  "Tipo",
 ...
 st.markdown("##### 1 - ANTEPRIMA PER INSERIRE MANUALE")
 st.info("OTM / OSM - Anteprima per inserire posizione manuale")

 col_a,col_b=st.columns([2,1])
 with col_a:
  st.markdown("**Anteprima mappa OTM**")
 ...
 with st.form("form_mappa"):
  st.markdown("**Dati postazione - Intestazione bassa**")
  c1,c2=st.columns(2)
  with c1:
  ...
  st.markdown("**Anteprima dentro form OTM**")
  try:
   lat_f=float(m5)
   lon_f=float(m6)
 ...
 # 3 - VISIONA POSTAZIONI CON OTM
 st.markdown("##### 3 - VISIONA POSTAZIONI - OTM OSM")
 ...
 if st.session_state.post:
  try:
   ...
   if not df_all.empty:
    ...
    st.caption("OTM / OSM - Tutte le postazioni")
  except:
   pass
  ...
  st.map(df_def)
  st.info("Mappa Varese OTM")

elif scelta=="Backup":
 torna()
 st.markdown("#### BACKUP - INTESTAZIONI BASSE")
 ...
 torna()
 st.markdown("#### "+scelta)
 st.info("Form "+scelta+" - Intestazione bassa")
