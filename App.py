...
if scelta=="Dashboard":
    ...
    if st.session_state.volontario_selezionato is not None:
        ...
        st.write(f"{vol.get('Cellulare','')} | {vol.get('Ruolo','')}")
    ...
    with st.form("form"):
        ...
        if salva_vol:
            if nome and cognome and cell and assoc:
                st.session_state.dati.append({"Nome":f"{nome} {cognome}","Cellulare":cell,"Comune":comune,"Ruolo":ruolo,"Associazione":assoc})
                save_json(FILE_DATI,st.session_state.dati); st.rerun()
    for idx, vol in enumerate(st.session_state.dati):
        if st.button(f"👤 {vol.get('Nome','')}",key=f"vol_{idx}"):
            st.session_state.volontario_selezionato=vol; st.session_state.volontario_idx=idx; st.rerun()

elif scelta=="Mappa Postazioni":
    torna_dashboard()
    st.markdown("## MAPPA POSTAZIONI - Libreria condivisa")
    st.info("Carica PNG - Clicca sul logo salvato per vederlo su Google / OSM / Waze")
    up=st.file_uploader("Carica logo PNG", type=["png","jpg","jpeg"], key="up_icon_post")
    if up:
        b64=img_to_b64(up)
        nome_icona=st.text_input("Nome logo", value=up.name.split(".")[0], key="nome_icona_post")
    ...
    with c1:
        ...
        try:
            ...
            st_folium(m,width=700,height=500,key="mappa_click")
        except: st.map(pd.DataFrame([{"lat":lat_focus,"lon":lon_focus}]),zoom=12)
        st.markdown("#### Apri su Google / OSM / Waze")
        if st.session_state.postazione_selezionata_per_mappa:
            p_sel=st.session_state.postazione_selezionata_per_mappa
            lat_sel=p_sel.get('Latitudine'); lon_sel=p_sel.get('Longitudine'); logo_sel=p_sel.get('Icona','📍')
            c_osm,c_gm,c_waze=st.columns(3)
            with c_osm: st.link_button(f"OSM - {logo_sel}", f"https://www.openstreetmap.org/?mlat={lat_sel}&mlon={lon_sel}#map=17/{lat_sel}/{lon_sel}", use_container_width=True)
    ...
    with c2:
        ...
        if b64_sel: st.image(f"data:image/png;base64,{b64_sel}",width=120)
        opzioni_logo=["📍 Default"] + [f"{ic['nome']}" for ic in st.session_state.icone_lib]
        sel_logo=st.selectbox("Scegli logo", opzioni_logo, index=0, key="sel_logo_post")
        if sel_logo!=st.session_state.map_logo_selezionato:
            st.session_state.map_logo_selezionato=sel_logo; st.rerun()
        with st.form("form_post"):
            ...
            if salva_post:
                if nome_post and lat_post and lon_post:
                    ...
                    st.session_state.postazioni.append(new); save_json(FILE_POST,st.session_state.postazioni); st.success("Salvata!"); st.rerun()
    st.divider()
    ...
    for idx, p in enumerate(st.session_state.postazioni):
        ...
        with c2: st.write(p.get('Comune',''))
        with c3:
            if st.button(f"Vedi su mappa",key=f"use_em_{idx}",use_container_width=True):
                st.session_state.postazione_selezionata_per_mappa=p; st.rerun()
    ...
    with st.form("form_em"):
        ...
        if salva_em:
            if com and via and az:
                st.session_state.interventi_lista.append({"Comune":com,"Via":via,"Azione":az,"Logo":st.session_state.map_logo_selezionato})
